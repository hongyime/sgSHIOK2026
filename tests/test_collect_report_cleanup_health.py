...
class CollectorTests(unittest.TestCase):
    ...
    def collect(self, results=None, *, environ=None, now=NOW):
    ...
    def test_future_stale_missing_or_arbitrary_database_timestamp_fails_closed(self):
        for stamp in (None, SENTINEL, (NOW + timedelta(seconds=5, microseconds=1)).isoformat(),
    ...
    def test_actual_clock_receipt_remains_unobserved_and_preserves_db_instant(self):
        local = datetime.fromisoformat("2026-09-15T04:28:33.978+00:00")
        database = "2026-09-15 04:28:33.987873+00"
        items = responses(initial=True)
        rows = json.loads(items[2].body)
        rows[0]["snapshot"]["database_now"] = database
        items[2] = response(rows, 201)
        result, transport = self.collect(items, now=local)
        self.assertEqual(len(transport.calls), 3)
        self.assertEqual(result["status"], "unobserved")
        self.assertEqual(result["observed_at"], local.isoformat())
        self.assertEqual(result["database_now"], "2026-09-15T04:28:33.987873+00:00")
        self.assertEqual(collector._checked_output(json.dumps(result).encode(), lambda: local), result)

    def test_collection_skew_does_not_extend_cleanup_age_or_tolerate_future_cleanup(self):
        database_now = NOW + timedelta(seconds=5)
        for verified, expected, reason in (
            (database_now - timedelta(hours=26), "healthy", None),
            (database_now - timedelta(hours=26, milliseconds=1), "blocked", None),
            (database_now + timedelta(microseconds=1), "invalid", "future_timestamp"),
        ):
            items = responses()
            rows = json.loads(items[2].body)
            ...
            self.assertEqual(result["reason"], reason)
            self.assertEqual(collector._checked_output(json.dumps(result).encode(), lambda: NOW), result)
...
class CliBoundaryTests(unittest.TestCase):
    ...
    def test_worker_output_tolerates_only_bounded_database_lead(self):
        for skew in (timedelta(milliseconds=9), timedelta(seconds=5)):
            result = self.good_output()
            result["database_now"] = (NOW + skew).isoformat()
            worker = subprocess.CompletedProcess([], 0, json.dumps(result).encode(), b"")
            with patch.object(collector.subprocess, "run", return_value=worker):
                self.assertEqual(collector._isolated_collect(clock=lambda: NOW), result)
        for observed, database in (
            (NOW, NOW + timedelta(seconds=5, microseconds=1)),
            (NOW - timedelta(seconds=4), NOW + timedelta(seconds=2)),
            (NOW + timedelta(microseconds=1), NOW + timedelta(seconds=5)),
        ):
            result = self.good_output()
            result.update(observed_at=observed.isoformat(), database_now=database.isoformat())
            worker = subprocess.CompletedProcess([], 0, json.dumps(result).encode(), b"")
            with patch.object(collector.subprocess, "run", return_value=worker):
                self.assertEqual(collector._isolated_collect(clock=lambda: NOW)["reason"], "worker_failure")
