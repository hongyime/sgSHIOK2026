...
def test_prepare_vercel_source_copies_only_selected_bundle(tmp_path: Path, monkeypatch):
    ...
    (data_dir / "scores").mkdir()
    (data_dir / "scores" / "index.json").write_text('{"AREA":["123456"]}', encoding="utf-8")
    (data_dir / "scores" / "AREA.json").write_text('[{"postal":"123456"}]', encoding="utf-8")
    (data_dir / "geom" / "h3").mkdir(parents=True)
    (data_dir / "geom" / "h3" / "cell.json").write_text('[{"postal":"123456"}]', encoding="utf-8")
    (data_dir / "transit").mkdir()
    (data_dir / "transit" / "pois.json").write_text('{"features":[]}', encoding="utf-8")
    ...
    assert (stage / "web" / "public" / "data" / "selected_bundle" / "scores" / "AREA.json.gz").is_file()
    assert not (stage / "web" / "public" / "data" / "selected_bundle" / "scores" / "AREA.json").exists()
    assert (stage / "web" / "public" / "data" / "selected_bundle" / "geom" / "h3" / "cell.json.gz").is_file()
    assert not (stage / "web" / "public" / "data" / "selected_bundle" / "geom" / "h3" / "cell.json").exists()
    assert (stage / "web" / "public" / "data" / "selected_bundle" / "transit" / "pois.json.gz").is_file()
    assert not (stage / "web" / "public" / "data" / "selected_bundle" / "transit" / "pois.json").exists()
