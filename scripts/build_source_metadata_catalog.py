...
def main() -> int:
    ...
    for path in paths:
        ...
        if content != committed:
            raise RuntimeError("STOP_INPUT_MISMATCH tracked metadata: " + path + " expected="
                               + hashlib.sha256(committed).hexdigest() + " actual=" + hashlib.sha256(content).hexdigest())
        anchors[path] = hashlib.sha256(content).hexdigest()
        data[path] = yaml.safe_load(content) if path.endswith(".yaml") else json.loads(content)
    output = root / "source-metadata-catalog.json"
    result = make_catalog(data[paths[0]], data[paths[1]], anchors, datetime.now(UTC).isoformat())
