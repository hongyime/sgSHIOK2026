...
try {
    ...
    if (-not $SkipBrowser) {
        ...
        $SmokeRoot = "..\qa\debug-runs\launch-check-$Timestamp"
        Invoke-ExternalStep -Label "Scored browser smoke" -FilePath "npm.cmd" -TimeoutSec 180 -ArgumentList @("--prefix", "web", "run", "qa:browser", "--", "--url", "http://127.0.0.1:$Port/", "--postals", "560231,560234,570234", "--out", "$SmokeRoot\scored\summary.json")
        Invoke-ExternalStep -Label "Mayflower MRT-only browser smoke" -FilePath "npm.cmd" -TimeoutSec 180 -ArgumentList @("--prefix", "web", "run", "qa:browser", "--", "--url", "http://127.0.0.1:$Port/", "--postal", "560231", "--transit-mode", "mrt_lrt", "--must-include", "Mayflower MRT Station", "--out", "$SmokeRoot\mayflower-mrt\summary.json", "--debug-port", "$($Port + 99)")
        Invoke-ExternalStep -Label "Route compare browser smoke" -FilePath "npm.cmd" -TimeoutSec 180 -ArgumentList @("--prefix", "web", "run", "qa:browser", "--", "--url", "http://127.0.0.1:$Port/", "--postal", "560109", "--route-mode", "both", "--must-include", "shortest segments", "--out", "$SmokeRoot\route-compare\summary.json", "--debug-port", "$($Port + 98)")
        Invoke-ExternalStep -Label "No-transit browser smoke" -FilePath "npm.cmd" -TimeoutSec 180 -ArgumentList @("--prefix", "web", "run", "qa:browser", "--", "--url", "http://127.0.0.1:$Port/", "--postal", "567754", "--expected-state", "no_transit", "--out", "$SmokeRoot\no-transit\summary.json", "--debug-port", "$($Port + 100)")
        Invoke-ExternalStep -Label "Not-yet-scored browser smoke" -FilePath "npm.cmd" -TimeoutSec 180 -ArgumentList @("--prefix", "web", "run", "qa:browser", "--", "--url", "http://127.0.0.1:$Port/", "--postal", "000104", "--expected-state", "not_yet_scored", "--out", "$SmokeRoot\not-yet-scored\summary.json", "--debug-port", "$($Port + 101)")
    }
