...
export function rankPanelDescription(rankMetric: RankMetric, rankPanelOpen: boolean): string {
  ...
  return "Nearby-address comparison for this evidence row; locked SHIOK score is unchanged.";
}
...
function scoreStateNote(score: ScoreRecord, transitMode: TransitAccessMode): string | null {
  ...
  if (score.state === "SCORED_PARTIAL") {
    return "Partial locked score: shelter-map evidence may still be present, but unavailable locked-score rows count as zero in the locked scoring rule.";
  }
  ...
  if (busFallback) {
    return "Locked score caveat: the locked bus score remains 0 because nearby direct bus service evidence is not connected to a verified shelter-map walk.";
  }
  return null;
}
  ...
  const evidenceRows: EvidenceBreakdownRow[] = score.subscores
    ? [
        ...
        },
        {
          ...
          meta: scoredMeta(score.subscores.access, "35% locked walk-to-transit", "Stop/exit walk score unavailable"),
