...
import type { MetricCapability, PublishedTransitOption } from '../lib/published-transit-options';
...
function isPublishedRoute(option: PublishedTransitOption | null): boolean {
...
export function walkMetrics(score: ScoreRecord | null, shortest = false, option?: PublishedTransitOption | null) {
  if (option !== undefined) {
...
function availabilityNotice(score: ScoreRecord | null, option?: PublishedTransitOption | null): string | null {
...
export function WalkSummary({ postal, score, shortest = false, option }: {
  ...
  return <section className={styles.walkSummary} aria-label="Walk summary" data-postal={postal}>
    <h2>Postal {postal}</h2>
    <p className={styles.walkDestination}>Walk to <strong>{name || 'Destination unavailable'}</strong></p>
