...
describe("score coverage copy", () => {
  it("formats the live-bundle availability disclosure from manifest counts", () => {
    expect(
      ...
      "Bundle score availability: 95,157 full scores out of 124,443; 29,286 records (roughly a quarter) do not show a full score: 18,983 with partial shelter-map evidence, 9,827 beyond current transit range, and 476 awaiting scoring."
    );
  });
  ...
  it("uses a percentage when the non-full share is not near a quarter", () => {
    expect(
      ...
      "Bundle score availability: 900 full scores out of 1,000; 100 records (10%) do not show a full score: 80 with partial shelter-map evidence, 15 beyond current transit range, and 5 awaiting scoring."
    );
  });
  ...
  it("falls back to the generic non-full copy when state counts are incomplete", () => {
    expect(formatScoreCoverageLine(manifestWithCounts(1000, 900))).toBe(
      "Bundle score availability: 900 full scores out of 1,000; 100 records (10%) do not show a full score."
    );
  });
