...
describe("score card copy", () => {
  it("distinguishes far connected shelter-map walks from disconnected walks", () => {
    ...
    expect(source).toContain("No MRT/LRT exit or bus stop qualifies within the locked 1.2 km transit range for this postal.");
    ...
    expect(source).not.toContain("Closest connected shelter-map walk found is about ${formatDistance(nearestM)}");
    ...
    expect(source).not.toContain("No shelter-map walk to ${transitModeLabel(transitMode)} was found within the locked 1.2 km transit range.");
  ...
  it("keeps browser smoke aligned with awaiting bundle-score copy", () => {
    ...
    expect(smokeSource).toContain('summary.cardText.includes("Some shelter-map evidence may still be available")');
    ...
    expect(source).toContain("Some shelter-map evidence may still be available");
  ...
  it("puts data freshness and heat proxy copy in the title card", () => {
    ...
    expect(source).toContain('placeholder="Enter 6-digit postal"');
    expect(source).toContain('aria-label="Enter 6-digit Singapore postal code"');
    expect(source).toContain('aria-label="Postal-code search"');
    ...
    expect(source).not.toContain("searchOneMapLocations(query)");
    expect(source).not.toContain("shouldQueryOneMap(query)");
    ...
    expect(source).toContain("No published shelter-map record found for this postal code.");
    ...
    expect(source).not.toContain("OneMap search is busy. Try again in a moment, or enter a 6-digit postal code.");
    ...
    expect(source).toContain('timeZone: "Asia/Singapore"');
  ...
  it("shows four display rows without changing the locked weights", () => {
    ...
    expect(source).not.toContain('aria-label="Nearby-address comparison"');
    ...
    expect(source).not.toContain("Choose nearby-address comparison view");
    ...
    expect(source).toContain("No comparable nearby addresses for ${rankSentenceMetricLabel(rankMetricLabel)}.");
    expect(source).toContain("No comparable nearby addresses for ${sentenceLabel}.");
    expect(source).not.toContain("No comparable full locked scores in this planning area.");
    expect(source).not.toContain("No comparable planning-area addresses for ${rankSentenceMetricLabel(rankMetricLabel)}.");
    ...
    expect(source).not.toContain("Mostly exposed to rain");
    expect(source).not.toContain("Good rain shelter coverage");
    expect(source).toContain("Mostly exposed walk");
    ...
    expect(proposalSource).not.toContain(
      "In this locked release, rain shelter and the heat estimate share mostly the same covered-walkway evidence."
    );
