...
describe("score card copy", () => {
  it("distinguishes far connected shelter-map walks from disconnected walks", () => {
    ...
    expect(source).toContain("No connected shelter-map walk to ${transitModeLabel(transitMode)} within range");
    ...
    expect(source).toContain("Transit target found");
    expect(source).not.toContain("Transit stop or exit found");
    ...
    expect(source).toContain("Transit targets exist, but the published shelter-map bundle has no connected shelter-map walk yet.");
    expect(source).not.toContain("Transit stops or exits exist, but the published shelter-map bundle has no connected shelter-map walk yet.");
    ...
    expect(source).toContain("No shelter-map walk to ${label} within locked transit range");
  ...
  it("puts data freshness and heat proxy copy in the title card", () => {
    ...
    expect(source).toContain(
      "Data freshness at the 28 Aug 2026 UTC manifest-only check: 10 sources current, 8 stale, 2 manual, and Overture Maps Addresses - Singapore candidate has unknown age. No upstream URLs were probed. The oldest current item is NParks Leaf Area Index, a freshness-only reference table at its 120-day threshold with less than 0.1 days until stale. Stale sources by days past threshold are Planning Area Boundaries (MP2019 No Sea), NParks Tracks, NParks Heritage Road Green Buffers, Covered Linkway, Pedestrian Overhead Bridge / Underpass, Traffic Signals, NParks Heritage Trees, and NParks Nature Ways. Stale-source refreshes require a new numbered input version, not an in-place frozen-v1 mutation."
    );
    ...
    expect(source).toContain("at its 120-day threshold with less than 0.1 days until stale");
  ...
  it("shows four display rows without changing the locked weights", () => {
    ...
    expect(source).toContain("Locked score inputs unavailable");
    expect(source).not.toContain("Locked terms unavailable");
    expect(source).not.toContain("Locked score incomplete");
    expect(source).toContain(
      "Partial locked score: shelter-map evidence may still be present, but unavailable score inputs count as zero in the locked formula."
    );
