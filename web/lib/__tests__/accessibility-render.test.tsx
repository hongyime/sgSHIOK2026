...
describe("rendered accessibility output", () => {
  ...
  it("renders search result announcements and assertive error alerts", () => {
    ...
    expect(searchResultsAnnouncement([], false, null, true)).toBe(
      "No published shelter-map record found for this postal code. The published shelter-map data is tied to the June 2020 address list. P19 v2 28 Aug 2026 public-source sample: 6 coordinate-backed HDB missing rows (SUN PLAZA SPRING and YISHUN BEACON, three postals each) plus 2 unverified MCST address candidates (CANAAN and MYRA) out of 976 sampled 2021-2026 public-source rows with postals: 0.61% confirmed, or 0.82% including address-quality warnings. This is sampled evidence, not a complete missing-address count or approval to replace the June 2020 address list."
    );
    expect(noResultsHtml).toContain("No published shelter-map record found for this postal code.");
    expect(noResultsHtml).not.toContain("Try another address spelling");
    expect(noResultsHtml).not.toContain("Try a 6-digit postal code.</p>");
  ...
  it("introduces the shelter-map panel before search", () => {
    ...
    expect(html).toContain("See how much of the walk to transit is covered, and where it is exposed.");
    ...
    expect(html).not.toContain("The published shelter-map data is tied to the June 2020 address list.");
    ...
    expect(html).not.toContain("Full locked scores: 95,157 of 124,443");
    ...
    expect(html).not.toContain("Search a OneMap address or 6-digit postal code to inspect covered-walkway ratio and exposed gaps on the walk to transit, plus the night-lighting map layer.");
  ...
  it("renders live status for shelter-map panel load, walk mode, stop selection, and planning-area comparison", () => {
    ...
    expect(html.indexOf("Shelter-map walk evidence 48% covered-walkway ratio")).toBeLessThan(
      html.indexOf("Locked score for sorting 72 out of 100.")
    );
  ...
  it("uses transit stop-or-exit fallback copy when a scored walk has no named best node", () => {
    ...
    expect(html).toContain("Transit stop or exit not named");
  ...
  it("explains when a searched postal has no published shelter-map walk", () => {
    ...
    expect(html).toContain("No shelter-map walk is published for this postal yet.");
  ...
  it("names known P19 recent-source misses when the selected postal matches the cached list", () => {
    ...
    expect(hdbHtml).toContain("No shelter-map walk is published for this postal yet.");
    ...
    expect(mcstHtml).toContain("No shelter-map walk is published for this postal yet.");
  ...
  it("renders the route exposure lead and four-row score presentation", () => {
    ...
    expect(html).not.toContain('aria-label="Nearby-address comparison"');
    ...
    expect(html).not.toContain("Choose nearby-address comparison view");
    ...
    expect(html).not.toContain("Same displayed value as rain shelter for this postal.");
  ...
  it("matches planning-area empty copy to the selected comparison view", () => {
    ...
    expect(rankEmptyMessage("rain", "Covered-walkway evidence")).toBe(
      "No comparable nearby addresses for covered-walkway evidence."
    );
    ...
    expect(evidenceHtml).toContain("No comparable nearby addresses for covered-walkway evidence.");
    expect(evidenceHtml).not.toContain("No comparable planning-area addresses for covered-walkway evidence.");
  ...
  it("renders exposed gap lengths with coordinates", () => {
    ...
    expect(html).toContain("All listed exposed gaps are shown.");
  ...
  it("explains no-transit records when no qualifying candidate was selected", () => {
    ...
    expect(html).toContain("Beyond 1.2 km locked range");
    expect(html).not.toContain("Outside locked transit range");
    expect(html).toContain("No MRT/LRT exit or bus stop qualifies within the locked 1.2 km transit range for this postal.");
    expect(html).toContain("<span>No full locked score</span><strong>Walk evidence</strong>");
  ...
  it("phrases mode-specific no-walk states as walks to transit stops or exits", () => {
    ...
    expect(html).toContain("No verified published walk to bus stop");
    expect(html).not.toContain("No connected shelter-map walk to bus stop within range");
    expect(html).toContain("No verified walk to bus stop is published. The record does not establish why.");
    expect(html).toContain("Distance or connection failure is not established by this record");
  ...
  it("renders direct bus fallback evidence instead of a false low-bus verdict", () => {
    ...
    expect(html).toContain(
      "Locked score note: the locked bus score remains 0 because the straight-line bus estimate is not a verified shelter-map walk."
    );
    expect(html).not.toContain("Locked score caveat:");
