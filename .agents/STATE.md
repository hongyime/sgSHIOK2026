...
Machine: `Prawn-E14`
Latest substantive commit: `628407f` (`fix: require confirmation for full-rescore activation`)
...
Status:
- P752 is complete and pushed: `scripts/full-rescore-production.ps1` now requires `-ConfirmActivation` before rewriting `web/data-bundle.json` unless `-SkipActivateBundle` is supplied.
- Evidence: `qa/verification/P752-full-rescore-activation-confirmation.md`.
- P736-P751 also remain complete: bounded geocode caches must be versioned, batch-plan blocks unversioned completed fills, lamp-overlay/postal-universe/report-writer tasks require confirmation, high-risk writer/network/deploy tasks fail closed at the runner, validate is documented as read-only, publish/activation/deploy wrappers pass required confirmations, full-rescore deploy requires a distinct production deploy approval, agent publish instructions match, postal-universe prep passes required confirmations/cache paths, the legacy direct geocode entry point is retired, geocode cache paths use `--db`, and README DataMall discovery copy matches latest recorded evidence.
- Checks: `uv run pytest tests/test_release_scripts.py tests/test_run.py tests/test_publish.py -q` passed 70/70; `uv run pytest -q --collect-only` collected 576; repo integrity passed; diff-check and protected-diff guard passed.
