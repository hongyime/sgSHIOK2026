...
Machine: `Prawn-E14`
Latest substantive commit: `edd41bf` (`fix: require confirmation for direct network debug rebuilds`)
...
Status:
- P762 is complete and pushed: direct `scripts.rebuild_network_debug` now requires `--confirm-network-debug` before reading QA input or writing compact network debug GeoJSON, and `run.py network-debug` forwards that module-owned confirmation.
- Evidence: `qa/verification/P762-network-debug-confirmation.md`.
- P736-P761 also remain complete: bounded geocode caches must be versioned, batch-plan blocks unversioned completed fills, lamp-overlay/postal-universe/report-writer tasks require confirmation, high-risk writer/network/deploy tasks fail closed at the runner, full OneMap wrappers require explicit approval, validate is documented as read-only, publish/activation/deploy wrappers pass required confirmations, full-rescore deploy and activation require distinct approvals, agent publish instructions match, postal-universe prep passes required confirmations/cache paths, legacy direct geocode and network entry points are guarded/retired, geocode cache paths use `--db`, README DataMall discovery copy matches latest recorded evidence, direct fetch ingest requires module-owned approval, direct bus-arrivals collection requires module-owned approval, direct bus API ingest requires module-owned approval, direct postal-universe build requires module-owned approval, direct lamp-overlay build requires module-owned approval, direct export writer subcommands require module-owned approval, and direct Overture probes require module-owned approval.
- Checks: `uv run pytest tests/test_rebuild_network_debug.py tests/test_run.py -q` passed 64/64; `uv run pytest -q --collect-only` collected 591; repo integrity passed; diff-check and protected-diff guard passed.
