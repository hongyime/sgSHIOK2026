...
Machine: `Prawn-E14`
Latest substantive commit: `3d9e3c6` (`feat: add sample postal entry point`)
...
- Checks: `uv run pytest tests/test_legacy_geocode.py -q` passed 2/2; `uv run pytest tests/test_legacy_diag_scripts.py -q` passed 1/1; `uv run pytest tests/test_legacy_script_diagnostics.py -q` passed 1/1; `uv run pytest tests/test_analysis_scripts.py -q` passed 23/23; `npm --prefix web test -- score-card-copy.test.ts` passed 16/16; `npm --prefix web test` passed 166/166; `uv run pytest -q --collect-only` collected 618; repo integrity passed; protected-diff guard passed.
