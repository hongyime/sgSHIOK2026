...
Machine: `Prawn-E14`
Latest substantive commit: `2bba70c` (`fix: require confirmation for direct export writers`)
...
Status:
- P760 is complete and pushed: direct `pipeline.export export-transit` now requires `--confirm-export`, direct `pipeline.export refresh-provenance` now requires `--confirm-refresh-provenance`, and `run.py` forwards those module-owned confirmations.
- Evidence: `qa/verification/P760-export-subcommand-confirmation.md`.
- P736-P759 also remain complete: bounded geocode caches must be versioned, batch-plan blocks unversioned completed fills, lamp-overlay/postal-universe/report-writer tasks require confirmation, high-risk writer/network/deploy tasks fail closed at the runner, full OneMap wrappers require explicit approval, validate is documented as read-only, publish/activation/deploy wrappers pass required confirmations, full-rescore deploy and activation require distinct approvals, agent publish instructions match, postal-universe prep passes required confirmations/cache paths, legacy direct geocode and network entry points are guarded/retired, geocode cache paths use `--db`, README DataMall discovery copy matches latest recorded evidence, direct fetch ingest requires module-owned approval, direct bus-arrivals collection requires module-owned approval, direct bus API ingest requires module-owned approval, direct postal-universe build requires module-owned approval, and direct lamp-overlay build requires module-owned approval.
- Checks: `uv run pytest tests/test_export.py tests/test_run.py -q` passed 124/124; `uv run pytest -q --collect-only` collected 588; repo integrity passed; diff-check and protected-diff guard passed.
