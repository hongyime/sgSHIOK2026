...
const out=resolve(root,'qa/revamp-r1/resume-20260912');
...
const data=http.createServer(async(req,res)=>{
  ...
  try{
    const path=decodeURIComponent(new URL(req.url,'http://localhost:4321').pathname);
...
const proxy=http.createServer((req,res)=>{
  const path=new URL(req.url,'http://127.0.0.1:4362').pathname;
  ...
  const port=path.startsWith('/data/')?4321:4361,hostname=port===4321?'localhost':'127.0.0.1';
...
let child;
const identity={buildId:build.buildId,snapshot:build.snapshot,pid:process.pid,dataPort:4321,nextPort:4361,proxyPort:4362,readOnly:true};
try{
  await listen(data,4321,'localhost');await listen(proxy,4362,'127.0.0.1');
  child=spawn(process.execPath,[resolve(root,'web/node_modules/next/dist/bin/next'),'start',resolve(build.snapshot,'web'),'-p','4361','-H','127.0.0.1'],{cwd:root,windowsHide:true,env:{...process.env,NEXT_TELEMETRY_DISABLED:'1'},stdio:['ignore','pipe','pipe']});
