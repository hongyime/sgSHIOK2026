...
writeFileSync(resolve(out,'source-check-3.json'),JSON.stringify({buildId:build.buildId,sources:build.sources.length,anchors:Object.keys(provenance.sources).length,matched:true})+'\n',{flag:'wx'});
...
const proxy=http.createServer((req,res)=>{
  const path=new URL(req.url,'http://127.0.0.1:4420').pathname;
  ...
  const port=path.startsWith('/data/')?4340:4419,hostname=port===4340?'localhost':'127.0.0.1';
...
let child;
const identity={buildId:build.buildId,snapshot:build.snapshot,pid:process.pid,dataPort:4340,nextPort:4419,proxyPort:4420,readOnlyData:true, allowedApi:'GET /api/onemap-route and /api/onemap-search', oneMapCredentialsConfigured:Boolean(credentials.ONEMAP_EMAIL && credentials.ONEMAP_PASSWORD)};
try{
  ...
  child=spawn(process.execPath,[resolve(root,'web/node_modules/next/dist/bin/next'),'start',resolve(build.snapshot,'web'),'-p','4419','-H','127.0.0.1'],{cwd:root,windowsHide:true,env:{...process.env,...credentials,NEXT_TELEMETRY_DISABLED:'1'},stdio:['ignore','pipe','pipe']});
  child.stdout.pipe(createWriteStream(resolve(out,'next-3.stdout.txt'),{flags:'wx'}));child.stderr.pipe(createWriteStream(resolve(out,'next-3.stderr.txt'),{flags:'wx'}));
  identity.nextPid=child.pid;writeFileSync(resolve(out,'preview-3.json'),JSON.stringify(identity,null,2)+'\n',{flag:'wx'});
