...
const out = "C:\\sgSHIOK2026\\qa\\revamp-r1\\selection-recovery-20260913\\observed-vqv2K4";
const profile = "C:\\sgSHIOK2026\\tmp\\layout-confirmation-browser-sHrkp6";
const origin = 'http://127.0.0.1:4418', hash = bytes => createHash('sha256').update(bytes).digest('hex');
...
try {
  ...
  function focus(){
    ...
    return{tag:e?.tagName,id:e?.id,text:e?.textContent?.trim(),label:e?.getAttribute('aria-label'),clip,box:r?{x:r.x,y:r.y,width:r.width,height:r.height}:null,hit,visible:fits&&hit&&s?.visibility==='visible'&&s.opacity!=='0',focusVisible:e?.matches(':focus-visible'),outline:s?.outlineStyle,outlineWidth:s?.outlineWidth};
  }
  async function tabTo(text,tag='BUTTON'){
    for(let i=0;i<40;i++){const f=await evaluate(focus);if(f.tag===tag&&(tag==='CANVAS'||f.text===text)){check('keyboard focus '+text,f.visible&&f.focusVisible,f);return f;}await key('Tab','Tab',9);}
  ...
  await tabTo('MRT/LRT exits');await shot('walk-controls-return');
  ...
  await key('Tab','Tab',9);report.keyboardExit=await evaluate(focus);check('keyboard can leave canvas',report.keyboardExit.tag!=='CANVAS'&&report.keyboardExit.visible&&report.keyboardExit.focusVisible,report.keyboardExit);await shot('keyboard-exit');
