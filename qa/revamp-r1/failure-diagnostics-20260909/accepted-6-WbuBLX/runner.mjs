...
function errorDisposition(event, network) {
  ...
  if (url) {
    ...
    let optionalTransit = false;
    try {
      const parsed = new URL(url);
      optionalTransit = parsed.origin === origin && !parsed.search && !parsed.hash
        && /^\/data\/[^/]+\/transit\/h3\/[0-9a-f]+\.json\.gz$/.test(parsed.pathname);
    } catch { /* Not an optional transit shard. */ }
    if (optionalTransit && missing) return { disposition: 'optional-data-missing', reason: 'Observed optional transit shard404; the existing reader returns an empty layer' };
...
async function driverSelfTests() {
  ...
  const optionalUrl = origin + '/data/pinned/transit/h3/886520db3dfffff.json.gz';
  const optionalError = { ...error, params: { entry: { ...error.params.entry, url: optionalUrl } } };
  cases.push(['optional transit shard404 remains recorded', optionalError, [{ ...optional, params: { response: { status: 404, url: optionalUrl } } }], 'optional-data-missing'],
    ['optional transit error without404 evidence', optionalError, [], 'unexpected']);
...
function receive(message) {
  ...
  if (method === 'Target.attachedToTarget') {
    ...
    trackEvent((async () => {
      ...
      // Initial worker HTTP responses use the explicit server-finish receipt instead.
...
async function runScenario(context, kind) {
  ...
  let actualResponses;
  if (kind === 'worker') {
    const status = await serverJson('/__qa/status');
    check('worker fault evidence belongs to the same QA server', sameServer(status));
    const since = Date.parse(context.receipt.startedAt);
    const workerPath = '/maplibre/6.1.0/maplibre-gl-worker.mjs';
    const observed = report.network.some(event => event.scenario === context.name && event.method === 'Network.requestWillBeSent'
      && event.params.request.url === origin + workerPath && event.params.wallTime * 1000 >= since);
    actualResponses = status.requests.filter(item => item.path === workerPath && item.mode === 'worker'
      && item.kind === 'injected-fault' && item.status === 503 && item.at >= since && item.responseFinishedAt >= item.at);
    check('worker request observed and exact server503 response finished during this scenario', observed && actualResponses.length > 0, actualResponses);
    context.receipt.faultEvidenceSource = 'CDP request observation plus same-server response-finish receipt; initial worker HTTP response is not completely observable through CDP';
  } else {
    actualResponses = await waitNetwork(context, kind + ' actual503 response', () => {
      const found = faultResponses(context, kind); return found.length ? found : null;
    });
    context.receipt.faultEvidenceSource = 'CDP HTTP response';
  }
  ...
  if (kind === 'geometry') {
    await viewport(context, 390, 667);
    await waitFor(context, 'healthy short mobile route and tiles', healthy);
    await capture(context, 'healthy-390x667', healthy);
