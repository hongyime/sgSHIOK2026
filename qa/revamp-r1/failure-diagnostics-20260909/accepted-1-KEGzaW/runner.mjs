...
function facts() {
  ...
  const button = name => buttons.find(node => node.textContent.trim() === name);
  ...
  const diagnostics = copies.map(copy => {
    ...
    return {
      copy: buttonFacts(copy), select: buttonFacts([...owner.querySelectorAll('button')].find(node => node.textContent.trim() === 'Select diagnostics')),
      hide: buttonFacts([...owner.querySelectorAll('button')].find(node => node.textContent.trim() === 'Hide diagnostics')),
...
async function click(context, name) {
  const point = await call(context, label => {
    const matches = [...document.querySelectorAll('button')].filter(node => node.textContent.trim() === label);
...
function validateDiagnostic(value, kind) {
  ...
  check(kind + ' readonly labelled fallback exactly matches clicked snapshot', value.diagnostics.length === 1
    && control.status === 'Copy failed. Select the diagnostics to copy them.' && control.textarea.present
...
async function navigate(context) {
  ...
  await waitFor(context, 'search usable', value => value.shell && value.inputUsable && value.submitUsable);
