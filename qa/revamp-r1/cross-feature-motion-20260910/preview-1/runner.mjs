...
const build=JSON.parse(readFileSync(resolve(root,'qa/revamp-r1/cached-release-20260908/cross-feature-motion-20260910-1/build.json')));
if(build.exitCode!==0||!build.protectedDataAbsent)throw Error('Invalid build');
const live=await fetch('http://127.0.0.1:4351/',{signal:AbortSignal.timeout(30000)});
if(!live.ok||!(await live.text()).includes(build.buildId))throw Error('Wrong actual Next build');
const identity={root,directory,pid:process.pid,origin:'http://127.0.0.1:4352',buildB:build.buildId,sourceB:build.snapshot,active:'B'};
...
const server=http.createServer((req,res)=>{
  ...
  const port=url.pathname.startsWith('/data/')?4321:4351,hostname=port===4321?'localhost':'127.0.0.1';
