...
try {
  ...
  check('live preview matches pinned build',status.ok && report.preview.buildB === built.buildId);
  ...
  holdRetry=true;
  await fragment('compare=1&postals=018956%2C018990%2C079908&transit=bus&active=079908');
  ...
  for(const requestId of heldRetry.splice(0)){
    ...
    catch(error){report.releases.push({requestId,error:error.message});}
  }
  ...
  await until('rail category ready',()=>evaluate(facts),f=>ready(f)&&f.category==='MRT/LRT',90000);
  f=await evaluate(facts);
  const other=f.controls.find(c=>c.label?.startsWith('Show postal ')&&c.label!=='Show postal 018956 on map'&&!c.disabled);
  ...
  await activate('Close comparison');await until('inspector restored',()=>evaluate(facts),f=>!f.panel&&ready(f));
