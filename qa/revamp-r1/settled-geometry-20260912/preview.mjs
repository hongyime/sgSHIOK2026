...
const out=resolve(root,'qa/revamp-r1/settled-geometry-20260912');
...
writeFileSync(resolve(out,'source-check.json'),JSON.stringify({buildId:build.buildId,sources:build.sources.length,anchors:Object.keys(provenance.sources).length,matched:true})+'\n',{flag:'wx'});
const dataRoot=realpathSync(resolve(root,'web/public/data'));
const data=http.createServer(async(req,res)=>{
  if(!['GET','HEAD'].includes(req.method)){res.writeHead(405).end();return;}
  try{
    const path=decodeURIComponent(new URL(req.url,'http://localhost:4322').pathname);
...
const proxy=http.createServer((req,res)=>{
  const path=new URL(req.url,'http://127.0.0.1:4374').pathname;
  ...
  const port=path.startsWith('/data/')?4322:4373,hostname=port===4322?'localhost':'127.0.0.1';
...
let child;
const identity={buildId:build.buildId,snapshot:build.snapshot,pid:process.pid,dataPort:4322,nextPort:4373,proxyPort:4374,readOnly:true};
try{
  await listen(data,4322,'localhost');await listen(proxy,4374,'127.0.0.1');
  child=spawn(process.execPath,[resolve(root,'web/node_modules/next/dist/bin/next'),'start',resolve(build.snapshot,'web'),'-p','4373','-H','127.0.0.1'],{cwd:root,windowsHide:true,env:{...process.env,NEXT_TELEMETRY_DISABLED:'1'},stdio:['ignore','pipe','pipe']});
  child.stdout.pipe(createWriteStream(resolve(out,'next.stdout.txt'),{flags:'wx'}));child.stderr.pipe(createWriteStream(resolve(out,'next.stderr.txt'),{flags:'wx'}));
  identity.nextPid=child.pid;writeFileSync(resolve(out,'preview.json'),JSON.stringify(identity,null,2)+'\n',{flag:'wx'});
