...
const out = mkdtempSync(resolve(root, 'qa/revamp-r1/settled-geometry-20260912/observed-'));
const profile = mkdtempSync(resolve(root, 'tmp/layout-confirmation-browser-'));
const origin = 'http://127.0.0.1:4374', hash = bytes => createHash('sha256').update(bytes).digest('hex');
const started = Date.now(), deadline = started + 360000, workEnd = deadline - 60000;
...
try {
  ...
  const build = JSON.parse(readFileSync(resolve(root, 'qa/revamp-r1/settled-geometry-20260912/build-1/build.json')));
  ...
  for (const cache of ['cold', 'warm']) {
    assert.ok(os.freemem() / 1048576 >= 1024, 'Host memory gate before navigation');
    const sample = { cache, firstSequence: report.entries.length };
    report.samples.push(sample);
    await send('Page.navigate', { url: origin + '/?debugMap=1&postal=018956' }, mainSession);
    await until(cache + ' selected current map', () => evaluate(facts), ready, 120000);
    sample.page = await evaluate(() => ({ timeOrigin: performance.timeOrigin, now: performance.now(),
      events: window.__loadingDiagnosis?.events, longTasks: window.__loadingDiagnosis?.longTasks,
    ...
    const screenshot = await send('Page.captureScreenshot', { format: 'png', captureBeyondViewport: false }, mainSession);
    const after = await evaluate(facts);
    const png = Buffer.from(screenshot.data, 'base64');
    writeFileSync(resolve(out, cache + '.png'), png, { flag: 'wx' });
    sample.capture = { before, after, bytes: png.length, sha256: hash(png) };
    sample.lastSequence = report.entries.length;
    check(cache + ' current map stable across capture', ready(before) && ready(after) && before.routeKey === after.routeKey);
    await evaluate(() => window.__loadingDiagnosis?.stop());
    console.log(JSON.stringify({ cache, stages: sample.page.events.filter(e => ['selected-text-visible','map-published','current-selected-route-visible','basemap-source-loaded'].includes(e.name)) }));
  }
  ...
  report.scope = 'One instrumented local cold/warm pair. No worker pause. Not representative phone or pure server/worker accounting. No pipeline/install/build or preview modifications.';
