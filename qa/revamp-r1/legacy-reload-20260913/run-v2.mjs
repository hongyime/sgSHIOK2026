...
let child,cleaned;
try{
  const syntax=spawnSync(process.execPath,['--check',resolve(dir,'browser-v2.mjs')],{cwd:root,windowsHide:true,encoding:'utf8',timeout:10000});assert.equal(syntax.status,0,syntax.stderr);
  const timeout=Math.min(240000,started+300000-Date.now()-60000);assert.ok(timeout>0);
  child=spawnSync(process.execPath,[resolve(dir,'browser-v2.mjs'),out,profile],{cwd:root,windowsHide:true,encoding:'utf8',timeout,maxBuffer:4*1024*1024,env:{...process.env,TEMP:profile,TMP:profile}});
