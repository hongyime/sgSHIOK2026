...
const DATA='/data/generated_20260805_prefer_scored_routed/';
...
export function allowedPath(raw) {
  ...
  return url.pathname==='/'||['/sw.js','/icon.svg','/favicon.ico'].includes(url.pathname)||url.pathname.startsWith('/_next/static/')||url.pathname.startsWith('/maplibre/6.4.1/')||url.pathname.startsWith(DATA);
}
...
export async function startReleaseServer({html,htmlHeaders,assets,currentOrigin,wallMs=180000,maxRequests=600,maxBytes=128*1024*1024}) {
  ...
  let release='old',origin,closed=false,totalBytes=0,inFlightBytes=0;
  ...
  const server=http.createServer((req,res)=>{
    ...
    const finish=(status,headers,bytes,source)=>{
      if(res.destroyed||res.writableEnded)return;
      if(totalBytes+bytes.length>maxBytes){stats.limitHit='bytes';Object.assign(receipt,{status:503,source:'limit'});res.writeHead(503).end();return;}
    ...
    if(closed||!['GET','HEAD'].includes(req.method)||!allowedPath(req.url)){finish(403,{},Buffer.alloc(0),'blocked');return;}
    const url=new URL(req.url,origin),data=url.pathname.startsWith(DATA);
    if(release==='old'&&!data){
    ...
    const upstream=http.request({hostname:current.hostname,port:current.port,path:req.url,method:'GET',headers:{host:current.host,accept:req.headers.accept??'*/*'},signal:controller.signal},response=>{
      ...
      response.on('data',bytes=>{
        count+=bytes.length;inFlightBytes+=bytes.length;
        if(count>8*1024*1024||totalBytes+inFlightBytes>maxBytes){stats.limitHit='in-flight bytes';controller.abort();return;}chunks.push(bytes);
      });
      response.on('end',()=>finish(response.statusCode,response.headers,Buffer.concat(chunks),data?'read-only-data':'current-preview'));
  ...
  const close=async()=>{if(closed)return;closed=true;clearTimeout(timer);for(const req of upstreams)req.destroy();server.closeAllConnections();await new Promise(done=>server.close(done));stats.closed=true;};
  ...
  return {origin,requests,stats,flip(){assert.equal(release,'old');assert.equal(closed,false);release='current';stats.releaseSwitches++;},close,
    snapshot(){return{release,totalBytes,inFlightBytes,activeUpstreams:upstreams.size,activeSockets:sockets.size,...stats};}};
}
