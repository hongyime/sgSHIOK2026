...
import { startReleaseServer } from './release-server.mjs';
...
async function until(name,get,accept,limit){const end=Math.min(workEnd,Date.now()+limit);while(Date.now()<end){const value=await get();report.last={name,value};if(accept(value))return value;await delay(200);}throw Error(name+' timeout');}
...
try{
  ...
  ws.onmessage=event=>{const m=JSON.parse(event.data),p=m.params;if(m.id){pending.get(m.id)?.(m.result,m.error);return;}
    ...
    if(m.method==='Target.attachedToTarget'&&['worker','service_worker'].includes(p.targetInfo.type))background((async()=>{for(const domain of ['Runtime','Network'])await send(domain+'.enable',{},p.sessionId);})());
  };
  ...
  report.cacheBefore=await cacheSnapshot();report.beforeReload=await evaluate(documentFacts);report.oldContext=currentContext;
  ...
  check('current HTML contains current build identity',server.requests.some(r=>r.source==='current-preview'&&r.destination==='document'&&r.status===200));
  ...
  const documentResponses=server.requests.filter(r=>r.source==='current-preview'&&r.destination==='document'&&r.status===200);check('new document equals current preview HTML',documentResponses.some(r=>r.sha256===sha(Buffer.from(report.currentHtml))));delete report.currentHtml;
  const received=report.events.filter(e=>e.sessionId===session&&e.phase==='reload'&&e.method==='Network.responseReceived'&&e.params.type==='Document').at(-1);
  ...
  check('postal never reentered after ordinary Reload',report.currentRoute.url.includes('postal=018956')&&report.newDocument.input==='018956');
  ...
  check('no provider API request',!server.requests.some(r=>r.url.startsWith('/api/')));
  ...
  report.ok=!!report.completed&&!report.failure&&!report.errors.length&&report.cleanup.verified&&report.server?.closed&&report.anchorsUnchanged&&!report.chromeTerminal?.unverified&&Date.now()<=totalEnd;
