...
async function shot(name, selected = true) {
  ...
  if (selected) check(name + ' stable screenshot route', [before, after].every(f => f.status === 'ready' && f.featureCount === 4 && f.renderedKeys.length === 1 && f.renderedKeys[0] === f.routeKey && f.basemap && f.tiles && !f.moving) && before.routeKey === after.routeKey && before.timeOrigin === after.timeOrigin);
  return after;
}
