...
try {
  ...
  if (mode === 'live') {
    ...
    const count = Number(r.stdout.match(/\b([0-9]+) passed(?:,| in)/)?.[1]);
    assert.ok(count >= 95);
    result.testsPassed = count;
    result.meaning = `${count - 41} evaluator/collector +41 docs/integrity =${count} tests. Subtests are not separate test functions.`;
  }
