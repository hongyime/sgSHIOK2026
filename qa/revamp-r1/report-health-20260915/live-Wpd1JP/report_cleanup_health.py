...
behind the evaluator's trusted clock. observed_at must never exceed that local
clock. ONLY database_now may lead observed_at and local evaluation time, by at
most five seconds inclusive, to accommodate bounded inter-machine clock skew.
Cleanup/run timestamps still must not exceed database_now, with zero tolerance.

Cleanup/run freshness uses an inclusive 26-hour boundary, as in migration
20260915010921, measured against max(local evaluation time, database_now).
Neither tolerated clock skew nor delayed evaluation may extend that boundary.
The database timestamp is never clamped or rewritten to local time.
...
MAX_DATABASE_AHEAD = timedelta(seconds=5)
...
def evaluate(snapshot: Any, *, now: datetime | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        database_now = _timestamp(data["database_now"])
        if database_now - observed_at > MAX_DATABASE_AHEAD or database_now - now > MAX_DATABASE_AHEAD:
            raise _Invalid("future_timestamp")
        if now - observed_at > MAX_SNAPSHOT_AGE or now - database_now > MAX_SNAPSHOT_AGE:
            raise _Invalid("stale_snapshot")
        evaluation_now = max(now, database_now)
