...
const report = { root: ROOT, hostname: process.env.COMPUTERNAME, startedAt: new Date(started).toISOString(),
  ...
  checks: [], events: [], blocked: [], droppedEvents: 0, passed: false };
...
try {
  ...
  await until('owned Next ready', async () => {
    try { const response = await fetch(previewIdentity.url, { signal: AbortSignal.timeout(3000) }); return response.status; }
    catch { return null; }
  }, status => status === 200, 30000);
  ...
  try {
    check('no captured browser events dropped', report.droppedEvents === 0, report.droppedEvents);
    ...
    check('final drained boundary has no limit or active requests', report.boundary?.stats.closed === true && !report.boundary.stats.limitHit && report.boundary.stats.activeUpstreams === 0 && report.boundary.stats.activeSockets === 0, report.boundary?.stats);
    if (previewIdentity) {
      const statusResponse = await fetch(previewIdentity.url + '__qa/status', { signal: AbortSignal.timeout(5000) });
      assert.equal(statusResponse.status, 200);
      const finalPreview = await statusResponse.json();
