...
const MUTABLE_CONTEXT = new Set(['.agents/STATE.md', 'web/app/api/onemap-route/route.ts', 'web/components/route-evidence-map.tsx']);
...
const originalAttemptPath = guarded(path.join(OUT, 'audit.mjs'));
const originalAttemptHash = '588c3cd1f99065ea28f8aac978c23f63dc4320b7f6c8c2f98d292d9403461826';
if (sha(fs.readFileSync(originalAttemptPath)) !== originalAttemptHash) fail('Original failed attempt changed');
const outputPath = guarded(path.join(OUT, 'report-2.json'));
...
function readPinned(entry) {
  ...
  if (!MUTABLE_CONTEXT.has(entry.path) && (sha(bytes) !== entry.sha256 || bytes.length !== entry.bytes)) fail(`EXPECTED HASH DIFFERS: ${entry.path}`);
  return bytes;
}
if (manifest.files.reduce((sum, row) => sum + row.bytes, 0) > manifest.limits.totalBytes) fail('Total-byte gate');
for (const entry of manifest.files) {
  const bytes = readPinned(entry);
  buffers.set(entry.path, bytes);
  consumed.push({ ...entry, baselineSha256: entry.sha256, sha256: sha(bytes), bytes: bytes.length, guard: MUTABLE_CONTEXT.has(entry.path) ? 'mutable_context_excluded_not_repinned' : 'original_immutable_anchor', verifiedBefore: !MUTABLE_CONTEXT.has(entry.path), verifiedAfter: false });
}
...
for (const row of consumed) { if (!MUTABLE_CONTEXT.has(row.path)) { readPinned(row); row.verifiedAfter = true; } }
if (sha(fs.readFileSync(originalAttemptPath)) !== originalAttemptHash) fail('Original failed attempt changed');
verifyAbsences();
if (sha(fs.readFileSync(inputPath)) !== inputHash || sha(fs.readFileSync(ownPath)) !== ownHash) fail('Audit files changed during run');
timeGate();
const report = {
  originalFailedAttempt: { script: 'audit.mjs', scriptSha256: originalAttemptHash, manifest: 'inputs.json', manifestSha256: inputHash, reason: 'STATE.md hash changed before diagnosis assertions ran; original attempt remains unchanged', expectedStateSha256: 'd75c7d65059657c531554e9bbe1f6e3ab585979034e56fe115586d5bc3bb71bb', observedStateSha256: '41e65db306ecd19954ca200d25e34585222a8173978269ee3a924fef1ef800c1', disposition: 'Parent authorized handoff edit; only concurrent mutable STATE/API/map component excluded from immutable guards, not repinned. All other original anchors retained.' },
  ...
  changedPaths: ['qa/revamp-r1/route-gap-20260913/audit-2.mjs', 'qa/revamp-r1/route-gap-20260913/report-2.json'],
};
