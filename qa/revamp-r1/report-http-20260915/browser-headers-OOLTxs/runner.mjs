...
const ownedEnvironment = {
  ...
  TEMP: resolve(profile, 'tmp'), TMP: resolve(profile, 'tmp'), USERPROFILE: profile,
  APPDATA: resolve(profile, 'appdata'), LOCALAPPDATA: resolve(profile, 'localappdata'), PROGRAMDATA: resolve(profile, 'programdata'),
};
...
try {
  ...
  receipt.commands.push({ file: CHROME, args, cwd: ROOT, windowsHide: true, environment: 'OS paths and fresh profile only; no inherited credentials' });
...
@{matchedBefore=$before;remaining=$after} | ConvertTo-Json -Compress`;
  ...
  console.log(JSON.stringify({ out, passed: receipt.passed, durationMs: receipt.durationMs, requests: receipt.requests.length, checks: receipt.checks, cleanup: receipt.cleanup, errors: receipt.errors }, null, 2));
