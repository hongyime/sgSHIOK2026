...
try {
  ...
  vite = await createVite({ configFile: false, root: base, cacheDir: resolve(out, 'cache'), logLevel: 'error',
    plugins: [{name:'fixture-icon',configureServer(server){server.middlewares.use((req,res,next)=>{if(req.url!=='/icon.svg')return next();res.setHeader('Content-Type','image/svg+xml');res.end(readFileSync(resolve(root,'web/app/icon.svg')));});}}],
