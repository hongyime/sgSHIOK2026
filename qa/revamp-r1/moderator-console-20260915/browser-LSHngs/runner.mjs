...
try {
  ...
  vite = await createVite({ configFile: false, root: base, cacheDir: resolve(out, 'cache'), logLevel: 'error',
    plugins: [{name:'fixture-icon',configureServer(server){server.middlewares.use((req,res,next)=>{if(req.url!=='/icon.svg')return next();res.setHeader('Content-Type','image/svg+xml');res.end(readFileSync(resolve(root,'web/public/icon.svg')));});}}],
    resolve: { alias: ['react','react/jsx-runtime','react/jsx-dev-runtime','react-dom','react-dom/client'].map(name => ({ find: new RegExp(`^${name}$`), replacement: r.resolve(name) })) }, server: { host: '127.0.0.1', port: 0, hmr: false, watch: null, fs: { strict: true, allow: [base, resolve(root,'web/components'),resolve(root,'web/lib'),resolve(root,'web/node_modules')], deny: ['**/.env*','**/public/data/**','**/raw/**','**/processed/**'] } } });
  ...
  await wait("!!document.querySelector('input[type=password]')"); await wait("document.querySelector('main img')?.naturalWidth>0");
  check('brand asset rendered',await value("document.querySelector('main img').naturalWidth>0")); await capture('login',320,667);
  ...
  check('selection focuses review heading', await value("document.activeElement.hasAttribute('data-review-heading')"));
  ...
  check('confirmation focuses decision heading', await value("document.activeElement.hasAttribute('data-decision-heading')"));
  check('no decision request before confirmation', await value("!fixture.requests.some(p=>p.endsWith('/decision'))"));
  await click('Save decision'); await wait("document.body.textContent.includes('Save not confirmed.')");
  check('uncertainty focuses recovery heading',await value("document.activeElement.hasAttribute('data-recovery-heading')")); await capture('uncertain',320,667);
