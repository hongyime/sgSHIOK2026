...
function facts() {
  ...
  let count = 0, tiles = false, moving = true, basemap = false;
  ...
  if (map) { tiles = map.areTilesLoaded(); moving = map.isMoving(); basemap = !!map.getSource('onemap') && map.isSourceLoaded('onemap'); }
  return { url: location.href, timeOrigin: performance.timeOrigin, input: input?.value ?? null, inputBounds: rect(input), busy: form?.getAttribute('aria-busy'),
    ...
    postal: summary?.getAttribute('data-postal'), metrics: metricValues, count, routeKey: debug?.routeKey, tiles, moving, basemap,
...
const healthy = f => f.postal === '018956' && f.input === '018956' && f.busy === 'false' && f.status === 'ready' && f.count === 4 && f.routeKey && f.tiles && f.basemap && !f.moving
...
async function capture(c, name, accepted = healthy) {
  await until(name + ' selected result ready', () => call(c, facts), accepted);
  await call(c, () => new Promise((done, reject) => {
    const map = window.__shiokRouteMap;
    const idle = () => { clearTimeout(timer); done(true); };
    const timer = setTimeout(() => { map.off('idle', idle); reject(Error('Map idle capture barrier expired')); }, 15_000);
    map.once('idle', idle); map.triggerRepaint();
  }));
  const before = await call(c, facts);
  check(name + ' idle barrier preserves selected result', accepted(before));
...
async function run(c, mode) {
  ...
  await capture(c, mode + '-hydrated-submit', f => f.postal === '079908' && f.input === '079908' && f.busy === 'false'
    && new URL(f.url).searchParams.get('postal') === '079908' && f.count === 0 && f.tiles && f.basemap && !f.moving
