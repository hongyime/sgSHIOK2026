...
async function race(name,first,second,roles=['service_role','service_role']){
  ...
  try{
    ...
    void a.then(()=>{holderFinished=true;},()=>{holderFinished=true;});
    const handshakes=[];let holderPid;
    for(let attempt=0;attempt<5;attempt++){
      const rows=firstTransport==='management'?await rest(`${label}_observe`,{})
        :(await query(`begin; set local role service_role; select public.${label}_observe() rows; rollback;`)).at(-1).rows;
      handshakes.push(rows);
      if(rows.length===1){holderPid=rows[0].pid;break;}
      if(holderFinished)break;
      await sleep(80);
    }
    result.handshakes??=[];result.handshakes.push({name,samples:handshakes,holderPid:holderPid??null});
    assert.ok(holderPid&&!holderFinished,'Holder lock not observed live; stop');
...
try{
  ...
  if(mode==='rest-pilot'){
    ...
    await query(`begin; set local statement_timeout='12s'; ${sources.map(source=>transform(source.sql)).join('\n')}
      ...
      create function public.${label}_observe() returns jsonb language sql stable security invoker set search_path='' as $$
        select coalesce(jsonb_agg(jsonb_build_object('pid',pid,'mode',mode,'granted',granted)),'[]'::jsonb)
        from pg_locks where relation='${schema}.control'::regclass and mode='RowShareLock' and granted; $$;
      revoke all on function public.${label}_observe() from public,anon,authenticated;
      grant execute on function public.${label}_observe() to service_role;
    ...
    const [observer]=await query(`select 'public.${label}_observe()'::regprocedure::oid oid;`,true);observeFunctionOid=observer.oid;result.observeFunctionOid=observeFunctionOid;
  ...
  try{
    ...
    if(result.schemaCreationAttempted){
      ...
      if(identity.oid!==null){assert.ok(schemaOid&&identity.oid===schemaOid,'Unknown QA schema identity; do not drop');
        ...
        const [observer]=await query(`select to_regprocedure('public.${label}_observe()')::oid oid;`,true);
        assert.equal(observer.oid,observeFunctionOid,'Unexpected QA observer identity; do not drop');
        await query(`drop function public.${label}_call(text,uuid,text,text,boolean); drop function public.${label}_observe(); drop schema ${schema} cascade; notify pgrst,'reload schema';`);
      }
      ...
      const [gone]=await query(`select to_regprocedure('public.${label}_call(text,uuid,text,text,boolean)') is null and to_regprocedure('public.${label}_observe()') is null absent;`,true);assert.equal(gone.absent,true);result.publicWrappersRemoved=true;
    }
