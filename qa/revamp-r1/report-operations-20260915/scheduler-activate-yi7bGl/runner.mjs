...
try{
  ...
  if(mode==='activate'){
    ...
    result.activation=await query(`do $$ declare result jsonb; j cron.job%rowtype; begin
      select * into j from cron.job where jobname='shiok-report-cleanup-v1' for update;
    ...
    from cron.job where jobname='shiok-report-cleanup-v1';`,false);
