...
try{
  ...
  vite=await createVite({configFile:false,root:base,cacheDir:resolve(out,'cache'),logLevel:'error',resolve:{alias:[{find:/^react$/,replacement:r.resolve('react')},{find:/^react-dom\/client$/,replacement:r.resolve('react-dom/client')}]},server:{host:'127.0.0.1',port:0,hmr:false,fs:{strict:true,allow:[base,resolve(root,'web/components'),resolve(root,'web/lib'),resolve(root,'web/node_modules')],deny:['**/.env*','**/public/data/**','**/raw/**','**/processed/**']}}});
  ...
  if(chrome){try{await bounded(closed,6000,'Chrome close');}catch{execFileSync('taskkill.exe',['/PID',String(chrome.pid),'/T','/F'],{cwd:root,windowsHide:true,env,timeout:10000});await bounded(closed,5000,'forced Chrome close');}}
  await vite?.close();for(const socket of sockets)socket.destroy();await new Promise(done=>proxy.close(done));
