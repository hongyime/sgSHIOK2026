...
try {
  ...
  check('no uncaught exception or rejection across all Documents', report.pageErrors.length === 0 && (await sample('final')).unhandled.length === 0, report.pageErrors);
} catch (error) {
  ...
  report.ok = !report.failure && !transportError && report.checks.every(c => c.pass) && report.cleanup.verified && report.elapsedMs <= 600_000;
