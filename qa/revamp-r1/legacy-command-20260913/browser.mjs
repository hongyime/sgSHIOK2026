...
const dir=resolve(root,'qa/revamp-r1/legacy-command-20260913'),out=process.argv[2],profile=process.argv[3];
...
try{
  ...
  report.operation='preservation-sentinels';await evaluate(async()=>{localStorage.setItem('__qa:reload-preserve','local-sentinel');sessionStorage.setItem('__qa:reload-preserve','session-sentinel');await(await caches.open('qa-unrelated-cache')).put('/__qa/preserve',new Response('cache-sentinel'));return true;});
  report.cacheBefore=await cacheSnapshot('before');report.beforeReload=await evaluate(documentFacts);report.oldContext=currentContext;report.mainFrame=(await send('Page.getFrameTree')).frameTree.frame.id;
