...
const report = { root, hostname: process.env.COMPUTERNAME, label, origin, out, profile, build: built.buildId,
  startedAt: new Date().toISOString(), checks: [], captures: [], samples: [], network: [], documents: [], errors: [], faults: [] };
...
let chrome, ws, sequence = 0, stderr = '', closing = false, blockGeometry = true;
...
function facts() {
  ...
  return { viewport:[innerWidth,innerHeight], url:location.href, documentMarker:window.__qaDocumentMarker,
    ...
    focus:{ tag:active?.tagName,label:active?.getAttribute('aria-label') || active?.textContent,rect:focusRect,visible:!!hit && (hit === active || active.contains(hit)),outline:active ? getComputedStyle(active).outline : null },
    ...
    overflow:document.documentElement.scrollWidth > innerWidth,manual:[...panel?.querySelectorAll('textarea') ?? []].map(n => ({value:n.value,selection:[n.selectionStart,n.selectionEnd]})) };
}
...
try {
  ...
  ws.onmessage = event => {
    const m = JSON.parse(event.data); if(m.id)pending.get(m.id)?.finish(m.result,m.error);
      ...
      report.faults.push({url:request.url,blocked:blockGeometry,time:Date.now()});
      const action=blockGeometry?send('Fetch.fulfillRequest',{requestId,responseCode:503,responseHeaders:[{name:'Content-Type',value:'application/json'}],body:Buffer.from('{"error":"QA geometry failure"}').toString('base64')}):send('Fetch.continueRequest',{requestId});
  ...
  await send('Fetch.enable',{patterns:[{urlPattern:'*/geom/h3/886520db15fffff.json*',requestStage:'Request'}]});
  await send('Page.addScriptToEvaluateOnNewDocument',{source:`window.__qaDocumentMarker=Date.now(); window.__qaDenySave=false; const original=Storage.prototype.setItem; Storage.prototype.setItem=function(k,v){if(window.__qaDenySave&&k==='shiok:comparison:v1')throw new DOMException('QA storage denied','QuotaExceededError');return original.call(this,k,v);}; Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText:async()=>{throw new DOMException('QA clipboard denied','NotAllowedError');}}});`});
  ...
  check('diagnostics identify geometry and exclude postals/URLs',f.manual[0].value.includes('geometry')&&!/018956|018990|079908|debugMap|https?:|QA storage/.test(f.manual[0].value),f.manual[0].value);
  ...
  const beforeRetry=await evaluate(facts); blockGeometry=false;
  await key('Enter','Enter',13);
  const duringRetry=await evaluate(facts); report.samples.push({name:'Retry focus',before:beforeRetry,during:duringRetry});
  check('Retry retains keyboard context during loading',duringRetry.focus.tag!=='BODY'&&duringRetry.focus.visible,duringRetry.focus,false);
  await until('retry recovered without reload',()=>evaluate(facts),f=>!f.controls.some(c=>c.label==='Retry postal 018990')&&f.controls.some(c=>c.label==='Show postal 018990 on map'&&!c.disabled)&&ready(f));
  ...
  f=await evaluate(facts);check('failed Save preserves shared URL and saved list',f.shared&&f.url.includes('compare=1')&&JSON.parse(f.saved).postals.join(',')==='079908'&&f.focus.label==='Save shortlist on this device'&&f.focus.visible,f);
  ...
  await fragment('compare=1&postals=018956%2C018990&transit=bus&active=018990');
  await until('shared second walk',()=>evaluate(facts),f=>f.shared&&f.selectedPostal==='018990'&&ready(f));
  ...
  check('no uncaught errors or external mutation',!report.errors.length&&!report.unexpectedClose&&report.network.every(r=>r.method==='GET'&&!new URL(r.url).pathname.startsWith('/api/')),report.errors);
  ...
  report.ok=!report.failure&&!report.errors.length&&!report.unexpectedClose&&report.checks.every(c=>c.pass)&&report.cleanup.verified&&report.anchorsUnchanged;
