...
const directory=resolve(root,'qa/revamp-r1/retained-tab-20260910',attempt);
...
const b=JSON.parse(readFileSync(resolve(root,'qa/revamp-r1/cached-release-20260908/cross-feature-focus-20260910-2/build.json')));
...
if(buildA!=='e8Hlhkml4c3i_uMGJdd3P'||b.buildId!=='lcoYvon9S455KiFb_Oc8K')throw Error('Unexpected build identity');
...
const live=await(await fetch('http://127.0.0.1:4342/__qa/status',{signal:AbortSignal.timeout(5000)})).json();
if(live.build!==b.buildId)throw Error('B preview mismatch');
let active='A',stopping=false;const requests=[],nonce=randomUUID();
const identity={root,directory,pid:process.pid,origin:'http://127.0.0.1:4344',buildA,buildB:b.buildId,workerHashes,target,targetBytes:targetBytes.length,targetSha256:sha(targetBytes),sourceB:b.snapshot};
...
const server=http.createServer((req,res)=>{
  ...
  const data=url.pathname.startsWith('/data/'),port=data||active==='A'?4321:4341,hostname=port===4321?'localhost':'127.0.0.1';
...
server.listen(4344,'127.0.0.1',()=>{writeFileSync(resolve(directory,'runner.mjs'),readFileSync(new URL(import.meta.url)),{flag:'wx'});writeFileSync(receipt,JSON.stringify(identity,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(identity));});
