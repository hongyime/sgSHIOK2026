...
try {
  ...
  const after = await until('old tab terminal outcome', () => evaluate(facts), f => f.timeOrigin !== retained.timeOrigin || /unavailable|reload page|Application error|client-side exception|This page couldn.t load/i.test(f.text), 45000);
  ...
  const reload = after.controls.find(c => !c.disabled && ['Reload page','Reload'].includes(c.label.trim()));
  ...
  if (after.timeOrigin === retained.timeOrigin) {
    if (reload) {
      const point = await evaluate(label => { const b = [...document.querySelectorAll('button')].find(n => n.textContent.trim() === label); const r = b.getBoundingClientRect(); return { x: r.x+r.width/2, y: r.y+r.height/2 }; }, reload.label.trim());
      await send('Input.dispatchMouseEvent', { type: 'mousePressed', button: 'left', clickCount: 1, ...point });
      await send('Input.dispatchMouseEvent', { type: 'mouseReleased', button: 'left', clickCount: 1, ...point });
      report.recoveryAction = 'Visible application button: ' + reload.label.trim();
  ...
  const afterReload = await until('Reload destination', () => evaluate(facts), f => f.timeOrigin !== retained.timeOrigin && f.basemap && f.tiles && !f.moving, 120000);
  report.samples.push({ name: 'Reload-destination', facts: afterReload });
  const preserved = afterReload.text.includes('Bayfront') || afterReload.url.includes('018956');
  check('application Reload preserves selected postal', preserved, afterReload.url, false);
  if (!preserved) {
    await shot('recovered-B-lost-selection');
    // A separate ordinary user search, never counted as upgrade recovery.
    const point = await evaluate(() => { const n = document.querySelector('input[placeholder="Enter 6-digit postal"]'); if (!n) throw Error('Visible postal input absent'); const r = n.getBoundingClientRect(); return { x: r.x+r.width/2, y: r.y+r.height/2 }; });
    await send('Input.dispatchMouseEvent', { type: 'mousePressed', button: 'left', clickCount: 1, ...point });
    await send('Input.dispatchMouseEvent', { type: 'mouseReleased', button: 'left', clickCount: 1, ...point });
    await send('Input.insertText', { text: '018956' });
    await send('Input.dispatchKeyEvent', { type: 'keyDown', key: 'Enter', code: 'Enter', windowsVirtualKeyCode: 13, text: '\r' });
    await send('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Enter', code: 'Enter', windowsVirtualKeyCode: 13 });
    report.manualSearch = 'Re-entered 018956 in visible postal input and pressed Enter; not preserved selection';
  }
  await until('B selected route after separate user search if needed', () => evaluate(facts), ready, 120000);
  await documents();
  ...
  report.outcome = report.retainedAcceptance.usefulWalkDetails && report.retainedAcceptance.explicitReload ? 'explicit recovery accepted' : report.retainedAcceptance.automaticNewDocument ? 'automatic outcome observed; inspect' : 'retained old tab lost walk view; global error Reload recovers';
