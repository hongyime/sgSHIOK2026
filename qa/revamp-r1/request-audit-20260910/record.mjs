...
const paths = ['transport.mjs', 'transport.test.mjs', 'browser.mjs'].map(p => folder + '/' + p).filter(p => existsSync(resolve(root, p)));
...
const args = mode === 'contracts' ? ['--test', resolve(root, folder, 'transport.test.mjs')] : ['-B', resolve(root, 'scripts/check_repo_integrity.py')];
