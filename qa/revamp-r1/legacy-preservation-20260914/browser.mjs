...
import { transientNavigationContext, preservesSelectionUrl, reloadedDocument } from '../legacy-reload-20260913/navigation-proof.mjs';
import { createCommandChannel } from '../legacy-command-20260913/cdp-commands.mjs';
...
const dir=resolve(root,'qa/revamp-r1/legacy-preservation-20260914'),out=process.argv[2],profile=process.argv[3];
...
let server,chrome,chromeClosed,ws,session,pageTarget,stderr='',channel,closing=false,watchdog,currentContext;
...
function send(method,params={},sid=session,timeout=10000){return channel.send(method,params,sid,timeout);}
...
async function shot(name,current=false){
  report.operation='screenshot:'+name+':before';const before=await evaluate(current?routeFacts:documentFacts);
  report.operation='screenshot:'+name+':pixels';const r=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});
  const bytes=Buffer.from(r.data,'base64');writeFileSync(resolve(out,name+'.png'),bytes,{flag:'wx'});
  report.operation='screenshot:'+name+':after';const after=await evaluate(current?routeFacts:documentFacts);
  report.captures.push({name,bytes:bytes.length,sha256:sha(bytes),before,after});
  if(current)check(name+' selected route stable',ready(before)&&ready(after)&&before.routeKey===after.routeKey&&before.url===after.url&&JSON.stringify(before.geometry)===JSON.stringify(after.geometry),{before,after});
}
async function cacheSnapshot(label){report.operation='cache:'+label;return evaluate(async()=>{
...
try{
  ...
  channel=createCommandChannel({write:value=>ws.send(value),remaining,context:()=>({phase:report.phase,operation:report.operation,contextId:currentContext}),now:()=>Date.now()-started});
  ws.onmessage=event=>{const m=JSON.parse(event.data),p=m.params;if(channel.receive(m))return;
  ...
  report.preservationSteps=[];
  await preparePreservation({evaluate,now:()=>Date.now()-started,remaining,operation:value=>{report.operation=value;},records:report.preservationSteps});
  report.cacheBefore=await cacheSnapshot('before');
  check('preservation baseline complete',validPreservationBaseline(report.cacheBefore,server.origin),report.cacheBefore);
  report.beforeReload=await evaluate(documentFacts);report.oldContext=currentContext;report.mainFrame=(await send('Page.getFrameTree')).frameTree.frame.id;
  ...
  report.cacheAfter=await cacheSnapshot('after');
  for(const entry of report.cacheBefore.entries){const next=report.cacheAfter.entries.find(e=>e.cache===entry.cache&&e.url===entry.url);check('preserved cached '+entry.cache+' '+new URL(entry.url).pathname,preservedCacheEntry(entry,next,{origin:server.origin,documentSha256:report.receivedDocument.sha256,documentBytes:report.receivedDocument.bytes}),{before:entry,after:next,mutableNavigationUpdated:next?.sha256!==entry.sha256});}
  ...
  report.failure=error.stack;report.failureCommandId=error.commandId??null;
  if(ws?.readyState===1&&remaining()>8000){
    report.operation='failure:browser-version';
    try{const v=await send('Browser.getVersion',{},'',3000);report.failureBrowserProbe={responded:true,product:v.product,protocolVersion:v.protocolVersion};}
    catch(probeError){report.failureBrowserProbe={responded:false,error:probeError.message};}
    if(remaining()>4000){
      report.operation='failure:unbracketed-pixels';
      try{const r=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false},session,4000);const bytes=Buffer.from(r.data,'base64');writeFileSync(resolve(out,'failure-unbracketed.png'),bytes,{flag:'wx'});report.unbracketedCapture={bytes:bytes.length,sha256:sha(bytes)};}
  ...
  ws?.close();channel?.close();if(server){await server.close();report.server={requests:server.requests,...server.snapshot()};}
  ...
  report.commandDiagnostics=channel?.snapshot();
  ...
  report.ok=!!report.completed&&!report.failure&&!report.errors.length&&report.cleanup.verified&&report.finalBoundsClear&&!report.commandDiagnostics?.limitHit&&report.commandDiagnostics?.pending===0&&report.anchorsUnchanged&&!report.chromeTerminal?.unverified&&Date.now()<=totalEnd;
