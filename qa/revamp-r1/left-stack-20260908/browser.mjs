...
const out = resolve(root, 'qa/revamp-r1/left-stack-20260908', name);
...
const chrome = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe', ['--headless=new','--no-first-run','--no-sandbox','--use-angle=swiftshader','--enable-unsafe-swiftshader','--remote-debugging-port=9846',`--user-data-dir=${resolve(root,'tmp/left-stack-'+Date.now())}`,'about:blank'], { windowsHide: true, stdio:['ignore','ignore','pipe'] });
report.browserStartup={pid:chrome.pid,port:9846,stderr:''};
chrome.stderr.on('data',chunk=>{report.browserStartup.stderr=(report.browserStartup.stderr+chunk.toString()).slice(-16000);});
chrome.on('error',e=>{report.browserStartup.error=e.message;});
...
try {
  ...
  const route=async()=>{await until(`window.__shiokRouteMap?.getLayer('shiokest-route-line') && !window.__shiokRouteMap.isMoving() && window.__shiokRouteMap.isSourceLoaded('shiokest-route') && window.__shiokRouteMap.queryRenderedFeatures({layers:['shiokest-route-line']}).some(f=>f.properties.render_key===window.__shiokRouteDebug.routeKey)`);await basemap();};
  ...
  const layout=f=>{check('brand above left search '+f.width,f.brand.x<=16&&f.brand.y<=16&&f.input.y>f.brand.bottom&&Math.abs(f.brand.x-f.input.x)<2);check('icon inside field '+f.width,f.submit.x>=f.input.x&&f.submit.right<=f.input.right+1&&f.submit.bottom<=f.input.bottom+1);check('bottom-right disclosure and visible attribution '+f.width,f.dock.right>=f.width-16&&f.dock.right<=f.width&&!f.overflow&&f.dock.bottom<=f.attribution.y&&f.attribution.bottom<=f.height);if(f.panel)check('result below search with same width '+f.width,f.panel.y>f.input.bottom&&Math.abs(f.panel.x-f.input.x)<2&&Math.abs(f.panel.width-f.input.width)<2);};
  ...
  for(const [width,height]of [[390,844],[390,667],[320,667],[1440,950]]){await send('Emulation.setDeviceMetricsOverride',{width,height,deviceScaleFactor:1,mobile:false});await delay(1200);await route();f=await capture(`selected-${width}x${height}`);layout(f);check('current selected route '+width,f.routeCount===4&&f.text.includes('81 m'));if(width<700)check('result clears About dock '+width,f.panel.bottom<f.dock.y);}
  await send('Emulation.setDeviceMetricsOverride',{width:320,height:667,deviceScaleFactor:1,mobile:false});await delay(900);await route();await evaluate(`document.querySelector('aside > button').click()`);await delay(900);await route();f=await capture('details-expanded-320x667');layout(f);check('expanded stack preserves usable map height',f.stack.bottom<=f.height*.72&&f.routeCount===4);await evaluate(`document.querySelector('aside > button').click()`);await delay(900);await route();
