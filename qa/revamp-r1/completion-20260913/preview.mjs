...
const data=http.createServer(async(req,res)=>{
  ...
  try{
    const path=decodeURIComponent(new URL(req.url,'http://localhost:4336').pathname);
...
const proxy=http.createServer((req,res)=>{
  const path=new URL(req.url,'http://127.0.0.1:4410').pathname;
  ...
  const port=path.startsWith('/data/')?4336:4409,hostname=port===4336?'localhost':'127.0.0.1';
...
let child;
const identity={buildId:build.buildId,snapshot:build.snapshot,pid:process.pid,dataPort:4336,nextPort:4409,proxyPort:4410,readOnlyData:true, allowedApi:'GET /api/onemap-route and /api/onemap-search', oneMapCredentialsConfigured:Boolean(credentials.ONEMAP_EMAIL && credentials.ONEMAP_PASSWORD)};
try{
  await listen(data,4336,'localhost');await listen(proxy,4410,'127.0.0.1');
  child=spawn(process.execPath,[resolve(root,'web/node_modules/next/dist/bin/next'),'start',resolve(build.snapshot,'web'),'-p','4409','-H','127.0.0.1'],{cwd:root,windowsHide:true,env:{...process.env,...credentials,NEXT_TELEMETRY_DISABLED:'1'},stdio:['ignore','pipe','pipe']});
