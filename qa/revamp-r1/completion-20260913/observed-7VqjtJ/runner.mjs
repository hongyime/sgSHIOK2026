...
const out = mkdtempSync(resolve(root, 'qa/revamp-r1/completion-20260913/observed-'));
const profile = mkdtempSync(resolve(root, 'tmp/layout-confirmation-browser-'));
const origin = 'http://127.0.0.1:4410', hash = bytes => createHash('sha256').update(bytes).digest('hex');
...
try {
  ...
  const build = JSON.parse(readFileSync(resolve(root, 'qa/revamp-r1/completion-20260913/build-1/build.json')));
  ...
  await capture('mobile-shortest-default');
  const suggested=JSON.stringify((await evaluate(facts)).geometry);
  check('baseline source geometry present',suggested && suggested !== '[]');
  for(const [label,mode] of [['MRT/LRT exits','mrt_lrt'],['Bus stops','bus']]){
    ...
    const geometry=JSON.stringify((await evaluate(facts)).geometry);
    check(label+' fixture route geometry matches selection',mode==='mrt_lrt'?geometry!==suggested:geometry===suggested);
  }
  check('Bus restores shortest saved bus geometry',JSON.stringify((await evaluate(facts)).geometry)===suggested);
  ...
  for(const [width,height] of [[320,667],[390,667],[1440,950]]){
  ...
  await send('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:false},mainSession);
  const state = await evaluate(ui);
  check('only two transit controls',state.controls.length===2 && state.controls.every(b=>['MRT/LRT exits','Bus stops'].includes(b.text)), state.controls);
  check('retired controls absent',!/(Suggested|About data|Compare|Night lighting)/.test(state.bodyText),state.bodyText);
  await until('automatic local lamp data',()=>evaluate(facts),f=>f.lampCount>0,45000);
  await capture('automatic-lighting');
  report.exitESetup = await evaluate(() => {
    const map = window.__shiokRouteMap;
    const poi = map.getStyle().sources['transit-pois'].data.features.find(f => f.properties.id === 'mrt:21677');
    if (!poi) throw Error('Audited Exit E missing from loaded POI data');
    map.jumpTo({ center: poi.geometry.coordinates, zoom: 16 });
    map.panBy([0, -180], { duration: 0 });
    return { id: poi.properties.id, coordinates: poi.geometry.coordinates };
  });
  await until('Exit E marker camera settles',()=>evaluate(facts),f=>f.tiles&&!f.moving,30000);
  const exitETarget=await evaluate(()=>{
    const map=window.__shiokRouteMap;
    const poi=map.queryRenderedFeatures().find(f=>f.source==='transit-pois'&&f.properties.id==='mrt:21677'&&f.geometry.type==='Point');
    if(!poi)return null;
    const p=map.project(poi.geometry.coordinates);
    return document.elementFromPoint(p.x,p.y)?.tagName==='CANVAS'?{x:p.x,y:p.y}:null;
  });
  check('audited saved Exit E has unobscured real marker',!!exitETarget,exitETarget);
  await send('Input.dispatchMouseEvent',{type:'mousePressed',...exitETarget,button:'left',clickCount:1},mainSession);
  await send('Input.dispatchMouseEvent',{type:'mouseReleased',...exitETarget,button:'left',clickCount:1},mainSession);
  await until('saved Exit E selection',()=>evaluate(ui),u=>u.destination?.includes('Exit E')&&u.pressed[0]==='MRT/LRT exits');
  await capture('saved-exit-e-marker');
  check('saved Exit E marker needs no provider request',(report.previewRequests??[]).length===0);
  await send('Page.navigate',{url:origin+'/?debugMap=1&postal=018956&transit=mrt_lrt&stop=mrt%3A21677'},mainSession);
  await until('saved Exit E deep link',()=>evaluate(ui),u=>u.destination?.includes('Exit E'));
  await capture('saved-exit-e-link');
  check('saved Exit E deep link needs no provider request',(report.previewRequests??[]).length===0);
  await tap('aside [aria-label="Transit stop or exit type"] button','Bus stops');
  await capture('bus-before-unsaved-preview');
  // Choose an actual on-map bus POI outside this postal's saved candidate set.
  ...
  for(const r of [fixtureScore,...Object.values(fixtureScore.route_options??{})]) {
    for(const v of Object.values(r.best_node??{})) if(typeof v==='string') known.add(v);
  }
  await send('Runtime.evaluate',{expression:'window.__qaKnown='+JSON.stringify([...known])},mainSession);
  report.targetSetup=await evaluate(()=>{
    const m=window.__shiokRouteMap;
    const f=m.getStyle().sources['transit-pois'].data.features.find(f=>f.properties.kind==='bus_stop'&&!window.__qaKnown.includes(f.properties.id));
    if(!f) throw Error('No unsaved bus in loaded POI shard');
    m.jumpTo({center:f.geometry.coordinates,zoom:16});
    m.panBy([0,-180],{duration:0});
    return {id:f.properties.id,coordinates:f.geometry.coordinates,method:'camera setup before actual marker click'};
  });
  await until('camera setup current route',()=>evaluate(facts),f=>f.tiles&&!f.moving,30000);
  report.visibleTransit=await evaluate(()=>window.__shiokRouteMap?.queryRenderedFeatures().filter(f=>f.source==='transit-pois').map(f=>({id:f.properties.id,coordinates:f.geometry.coordinates})));
  const target=await evaluate(()=>{
    const m=window.__shiokRouteMap;
    const features=m.queryRenderedFeatures().filter(f=>f.source==='transit-pois'&&f.geometry.type==='Point'&&f.properties.kind==='bus_stop'&&!window.__qaKnown.includes(f.properties.id));
    for(const f of features) {
      const p=m.project(f.geometry.coordinates), e=document.elementFromPoint(p.x,p.y);
      if(e?.tagName==='CANVAS' && p.y>100 && p.y<innerHeight-30 && p.x>20 && p.x<innerWidth-20)
        return {x:p.x,y:p.y,id:f.properties.id};
    }
    return null;
  });
  check('real unsaved bus click target exists',!!target,target);
  report.unsavedTarget=target;
  const beforePreview=await evaluate(facts);
  report.beforePreview=beforePreview;
  await send('Input.dispatchMouseEvent',{type:'mousePressed',x:target.x,y:target.y,button:'left',clickCount:1},mainSession);
  await send('Input.dispatchMouseEvent',{type:'mouseReleased',x:target.x,y:target.y,button:'left',clickCount:1},mainSession);
  await until('explicit unavailable preview',()=>evaluate(ui),u=>u.bodyText.includes('Online preview unavailable'),30000);
  check('one user click one provider attempt',report.previewRequests?.length===1,report.previewRequests);
  check('failed preview retains complete saved geometry',JSON.stringify((await evaluate(facts)).geometry)===JSON.stringify(beforePreview.geometry));
  await capture('bus-preview-unavailable');
  function popupFacts() {
    ...
    return { count:document.querySelectorAll('.maplibregl-popup').length, open:details?.open,
      text:popup?.innerText, rowsVisible:dl?.checkVisibility({contentVisibilityAuto:true,visibilityProperty:true}),
      overflow:document.documentElement.scrollWidth>innerWidth,
      box:rect?{left:rect.left,right:rect.right,top:rect.top,bottom:rect.bottom}:null,
      viewport:[innerWidth,innerHeight], active:document.activeElement?.tagName,
      redundantAction:[...(popup?.querySelectorAll('button')??[])].some(b=>b.textContent==='Walk here') };
  }
  const collapsedPopup=await evaluate(popupFacts);
  check('one compact popup without duplicate route action',collapsedPopup.count===1&&!collapsedPopup.open&&!collapsedPopup.rowsVisible&&!collapsedPopup.redundantAction,collapsedPopup);
  await send('Emulation.setDeviceMetricsOverride',{width:320,height:667,deviceScaleFactor:1,mobile:false},mainSession);
  await capture('popup-collapsed-320');
  await tap('.maplibregl-popup summary','Service details');
  await until('popup disclosure opens',()=>evaluate(popupFacts),p=>p.open&&p.rowsVisible);
  await capture('popup-expanded-320');
  const expandedPopup=await evaluate(popupFacts);
  check('expanded popup fits small screen',!expandedPopup.overflow&&expandedPopup.box.left>=0&&expandedPopup.box.right<=320&&expandedPopup.box.top>=0&&expandedPopup.box.bottom<=667,expandedPopup);
  check('popup disclosure does not repeat provider call',report.previewRequests.length===1);
  await send('Input.dispatchKeyEvent',{type:'keyDown',key:'Enter',code:'Enter',windowsVirtualKeyCode:13},mainSession);
  await send('Input.dispatchKeyEvent',{type:'keyUp',key:'Enter',code:'Enter',windowsVirtualKeyCode:13},mainSession);
  await until('keyboard closes disclosure',()=>evaluate(popupFacts),p=>p.open===false);
  check('popup keyboard action does not repeat provider call',report.previewRequests.length===1);
  await send('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:false},mainSession);

  await tap('button','Retry preview');
  await until('one explicit retry completed',()=>evaluate(ui),u=>u.bodyText.includes('Online preview unavailable')&&report.previewRequests?.length===2,30000);
  await tap('button','Back to saved walk');
  await capture('bus-preview-recovered');
  check('back clears preview failure without third request',!(await evaluate(ui)).bodyText.includes('Online preview unavailable')&&report.previewRequests.length===2);
