...
try {
  ...
  const popup=await evaluate(()=>{
    ...
    return {count:document.querySelectorAll('.maplibregl-popup').length,text:p?.innerText,secondary:!!p?.querySelector('details,dl'),
      closeFocused:document.activeElement?.classList.contains('maplibregl-popup-close-button'),
      box:rect?{left:rect.left,right:rect.right,top:rect.top,bottom:rect.bottom}:null};
  });
  check('compact Exit E popup has no secondary panels',popup.count===1&&!popup.secondary&&popup.text.includes('Exit E'),popup);
  check('native popup close receives keyboard focus',popup.closeFocused,popup);
  await send('Input.dispatchKeyEvent',{type:'rawKeyDown',key:'Enter',code:'Enter',windowsVirtualKeyCode:13},mainSession);
  await send('Input.dispatchKeyEvent',{type:'char',text:'\r',unmodifiedText:'\r'},mainSession);
  await send('Input.dispatchKeyEvent',{type:'keyUp',key:'Enter',code:'Enter',windowsVirtualKeyCode:13},mainSession);
  await until('keyboard closes popup',()=>evaluate(()=>document.querySelectorAll('.maplibregl-popup').length),n=>n===0,10000);
  check('closing popup does not request a route',(report.previewRequests??[]).length===0);
