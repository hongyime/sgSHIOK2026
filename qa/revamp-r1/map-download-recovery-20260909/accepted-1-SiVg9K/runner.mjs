...
if (extra || !/^[a-z0-9-]{1,60}$/.test(label || '') || !/^[\w-]{1,100}$/.test(build || '')) throw Error('Fresh label and expected build required');
...
const report = {
  root, hostname: process.env.COMPUTERNAME, out, build, startedAt: new Date(started).toISOString(),
  ...
  policy: 'One owned Chromium/SwiftShader process, three fresh browser contexts, real implementation-chunk faults. Network.setBypassServiceWorker=true and Network.setCacheDisabled=true throughout: NO service-worker upgrade or service-worker cache recovery coverage. Page-target network and runtime events; not complete worker transfer accounting, physical-device evidence or representative latency. No pipeline, install, deployment or API calls.',
...
async function capture(context, name, predicate) {
  const before = await sample(context, name + ' before');
  ...
  const signature = value => JSON.stringify([value.documentTimeOrigin, value.status, value.stage, value.reason, value.summary, value.routeKey, value.featureCount, value.basemap, value.tilesLoaded, value.moving]);
  report.captures.push({ scenario: context.name, name, path, bytes: bytes.length, sha256: sha(bytes), before, after });
  if (predicate) check(name + ' matching screenshot facts', predicate(after) && signature(before) === signature(after), { before, after });
}
...
async function createContext(name, target, fault) {
  ...
  const receipt = { name, browserContextId, target, fault, freshContext: true, cacheDisabled: true, bypassServiceWorker: true,
    samples: [], interceptions: [], documents: [], startedAt: new Date().toISOString() };
  report.scenarios.push(receipt);
  const context = { name, receipt, browserContextId, target, fault, held: new Map(), responses: new Map(), finished: new Set(), documents: new Map() };
...
async function holdScenario(context) {
  ...
  await capture(context, 'explicit-reload-ready', selectedReady);
  check('recovered page has no unhandled hint rejections', ready.unhandledRejections.length === 0, ready.unhandledRejections);
}
async function rejectionScenario(context, stage) {
  ...
  await capture(context, 'rejected-with-walk', value => walkIntact(value) && failedAs(value, stage, 'rejected'));
  check(context.name + ' stays in the same Document without automatic reload', failed.documentTimeOrigin === walk.documentTimeOrigin && context.documents.size === 1);
  check(context.name + ' no unhandled preload rejections', walk.unhandledRejections.length === 0, walk.unhandledRejections);
}
