...
async function clickControl(selector){
  const rect=await evaluate(`(()=>{const e=document.querySelector(${JSON.stringify(selector)});if(!e||e.disabled)return null;const r=e.getBoundingClientRect(),x=r.x+r.width/2,y=r.y+r.height/2,hit=document.elementFromPoint(x,y);return{x,y,width:r.width,height:r.height,inViewport:x>=0&&y>=0&&x<innerWidth&&y<innerHeight,hit:!!hit&&(hit===e||e.contains(hit))}})()`);
  check('visible enabled control '+selector,rect&&rect.width>0&&rect.height>0&&rect.inViewport&&rect.hit,rect);
  for(const type of ['mousePressed','mouseReleased'])await send('Input.dispatchMouseEvent',{type,x:rect.x,y:rect.y,button:'left',clickCount:1},session);
}
...
try{
  ...
  // This captured build reads stop from the URL, but needs the real search form
  // to select a postal. Do not substitute current-app deep-link behavior.
  await until('search document complete',()=>evaluate('document.readyState'),s=>s==='complete',10000);
  await pause(1000);
  report.search={postal:'018956',action:'one native form input and submit; no programmatic route selection',startedAt:new Date().toISOString()};
  await clickControl('#postal-search-input');
  await send('Input.insertText',{text:report.search.postal},session);
  report.search.input=await evaluate('document.querySelector("#postal-search-input")?.value');
  check('typed postal is visible',report.search.input===report.search.postal,report.search.input);
  report.search.beforeSubmitReceiptId=Math.max(0,...boundary.requests.map(r=>r.id));
  await clickControl('#postal-search-button');
  report.search.submittedAt=new Date().toISOString();
  const searchRequest=await until('real search initiated local data read',()=>boundary.requests.find(r=>r.id>report.search.beforeSubmitReceiptId&&r.method==='GET'&&r.sourceKind==='data'),Boolean,15000);
  report.search.firstDataRequest=structuredClone(searchRequest);
  ...
  report.search.firstDataRequest=structuredClone(boundary.requests.find(r=>r.id===searchRequest.id));
