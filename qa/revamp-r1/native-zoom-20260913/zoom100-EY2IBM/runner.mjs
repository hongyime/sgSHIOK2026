...
try {
  ...
  async function tabTo(text){for(let i=0;i<65;i++){const f=await evaluate(focusFacts);if(f.tag==='BUTTON'&&f.text===text){check('keyboard target visible '+text,f.visible&&f.focusVisible,f);return;}await key('Tab','Tab',9);}throw Error('Unreachable keyboard target '+text);}
  ...
  // Enumerate only displayed details controls; Tab itself must bring each into view.
  ...
  const gap=buttons.find(b=>/^Exposed \d/.test(b.text));
  if(gap){await tabTo(gap.text);await key('Enter','Enter',13);await capture('gap-selected');}
