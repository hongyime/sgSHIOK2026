...
const out = mkdtempSync(resolve(root, 'qa/revamp-r1/native-zoom-20260913/zoom200-'));
const profile = mkdtempSync(resolve(root, 'tmp/layout-confirmation-browser-'));
const origin = 'http://127.0.0.1:4414', hash = bytes => createHash('sha256').update(bytes).digest('hex');
...
try {
  ...
  const build = JSON.parse(readFileSync(resolve(root, 'qa/revamp-r1/keyboard-recovery-20260913/build-1/build.json')));
  ...
  const prefs={partition:{default_zoom_level:{x:3.8017840169239308}}};
  mkdirSync(resolve(profile,'Default'));
  writeFileSync(resolve(profile,'Default/Preferences'),JSON.stringify(prefs),{flag:'wx'});
  report.nativeZoom={requestedPercent:200,preferences:prefs,method:'Fresh owned profile native partition default zoom before Chrome launch; no page-scale/device-metrics override'};
  ...
  check('native zoom has expected physical/CSS ratio',Math.abs(metrics.outer[0]/metrics.inner[0]-2)<0.03&&Math.abs(metrics.dpr-2)<0.03&&metrics.visualScale===1,metrics);
  ...
  const alternate=await evaluate(()=>[...document.querySelectorAll('[aria-label="Published walks"] button')].find(e=>e.getAttribute('aria-pressed')==='false')?.textContent.trim());
  check('real nondefault saved choice exists',!!alternate,alternate);
  await tabTo(alternate);await key('Enter','Enter',13);await until('nondefault saved walk',()=>evaluate(()=>!!document.querySelector('[aria-label="Use published default"]')),Boolean,10000);
  ...
  check('reset retains visible surviving focus',!['BODY','HTML'].includes(report.resetFocus.tag)&&report.resetFocus.visible&&report.resetFocus.focusVisible,report.resetFocus);
