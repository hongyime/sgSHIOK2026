...
const out = resolve(root, 'qa/revamp-r1/worker-alignment-20260910/browser');
mkdirSync(out);
const profile = mkdtempSync(resolve(root, 'tmp/layout-confirmation-browser-'));
const origin = 'http://127.0.0.1:4362';
const report = { origin, profile, checks: [], captures: [], responses: [], failures: [], errors: [], startedAt: new Date().toISOString() };
...
function facts() {
  ...
  return { width: innerWidth, height: innerHeight, key: debug?.routeKey, count: features.filter(f => f.properties.render_key === debug?.routeKey).length,
    basemap: map?.isSourceLoaded('onemap'), tiles: map?.areTilesLoaded(), moving: map?.isMoving(), center: map?.getCenter().toArray(), zoom: map?.getZoom(),
