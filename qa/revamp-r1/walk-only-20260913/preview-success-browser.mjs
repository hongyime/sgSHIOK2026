...
try {
  ...
  ws.onmessage = event => {
    ...
    if (m.method === 'Fetch.requestPaused') {
      ...
      if (u.origin === origin && u.pathname === '/api/onemap-route') {
        ...
        report.previewRequests.push({ method: p.request.method, path: u.pathname, scenario: 'controlled replay of observed OneMap HTTP 200' });
        void send('Fetch.fulfillRequest', { requestId: p.requestId, responseCode: 200,
          responseHeaders: [{ name: 'Content-Type', value: 'application/json' }],
          body: Buffer.from(JSON.stringify(JSON.parse(readFileSync(resolve(root, 'qa/revamp-r1/walk-only-20260913/provider-probe.json'))).body)).toString('base64') }, sessionId)
  ...
  await capture('preview-success-baseline');
  ...
  await send('Input.dispatchMouseEvent',{type:'mousePressed',x:target.x,y:target.y,button:'left',clickCount:1},mainSession);
  await send('Input.dispatchMouseEvent',{type:'mouseReleased',x:target.x,y:target.y,button:'left',clickCount:1},mainSession);
  await until('successful preview owns map',()=>evaluate(facts),f=>ready(f)&&JSON.stringify(f.geometry)!==JSON.stringify(beforePreview.geometry),30000);
  await capture('actual-provider-payload-rendered');
  const previewUi=await evaluate(ui);
  check('preview is labelled, not a saved authoritative walk',/preview/i.test(previewUi.panelText),previewUi.panelText);
  check('preview succeeds without failure banner',!previewUi.bodyText.includes('Online preview unavailable'),previewUi.panelText);
  check('single explicit request',report.previewRequests.length===1,report.previewRequests);
  await tap('aside [aria-label="Transit stop or exit type"] button','Bus stops');
  await capture('return-from-successful-preview');
  check('saved bus restored after successful preview',JSON.stringify((await evaluate(facts)).geometry)===JSON.stringify(beforePreview.geometry));
