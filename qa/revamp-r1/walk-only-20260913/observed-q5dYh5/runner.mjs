...
const out = mkdtempSync(resolve(root, 'qa/revamp-r1/walk-only-20260913/observed-'));
const profile = mkdtempSync(resolve(root, 'tmp/layout-confirmation-browser-'));
const origin = 'http://127.0.0.1:4406', hash = bytes => createHash('sha256').update(bytes).digest('hex');
...
try {
  ...
  const build = JSON.parse(readFileSync(resolve(root, 'qa/revamp-r1/walk-only-20260913/build-2/build.json')));
