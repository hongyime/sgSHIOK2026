...
try {
  ...
  await capture('preview-baseline');
  ...
  await send('Input.dispatchMouseEvent',{type:'mouseWheel',x:350,y:620,deltaX:0,deltaY:450},mainSession);
  await until('wheel zoom settles',()=>evaluate(()=>window.__shiokRouteMap?.isMoving()===false),Boolean,15000);
  report.visibleTransit=await evaluate(()=>window.__shiokRouteMap?.queryRenderedFeatures().filter(f=>f.source==='transit-pois').map(f=>f.properties));
  const target=await evaluate(()=>{
