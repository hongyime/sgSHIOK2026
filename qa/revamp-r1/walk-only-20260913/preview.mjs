...
writeFileSync(resolve(out,'source-check-1.json'),JSON.stringify({buildId:build.buildId,sources:build.sources.length,anchors:Object.keys(provenance.sources).length,matched:true})+'\n',{flag:'wx'});
const dataRoot=realpathSync(resolve(root,'web/public/data'));
const data=http.createServer(async(req,res)=>{
  if(!['GET','HEAD'].includes(req.method)){res.writeHead(405).end();return;}
  try{
    const path=decodeURIComponent(new URL(req.url,'http://localhost:4334').pathname);
...
const proxy=http.createServer((req,res)=>{
  const path=new URL(req.url,'http://127.0.0.1:4404').pathname;
  if(!['GET','HEAD'].includes(req.method)||(path.startsWith('/api/') && !(req.method==='GET' && path==='/api/onemap-route'))){res.writeHead(403).end();return;}
  ...
  const port=path.startsWith('/data/')?4334:4403,hostname=port===4334?'localhost':'127.0.0.1';
...
let child;
const identity={buildId:build.buildId,snapshot:build.snapshot,pid:process.pid,dataPort:4334,nextPort:4403,proxyPort:4404,readOnlyData:true, allowedApi:'GET /api/onemap-route', oneMapCredentialsConfigured:Boolean(credentials.ONEMAP_EMAIL && credentials.ONEMAP_PASSWORD)};
try{
  await listen(data,4334,'localhost');await listen(proxy,4404,'127.0.0.1');
  child=spawn(process.execPath,[resolve(root,'web/node_modules/next/dist/bin/next'),'start',resolve(build.snapshot,'web'),'-p','4403','-H','127.0.0.1'],{cwd:root,windowsHide:true,env:{...process.env,...credentials,NEXT_TELEMETRY_DISABLED:'1'},stdio:['ignore','pipe','pipe']});
  child.stdout.pipe(createWriteStream(resolve(out,'next-1.stdout.txt'),{flags:'wx'}));child.stderr.pipe(createWriteStream(resolve(out,'next-1.stderr.txt'),{flags:'wx'}));
  identity.nextPid=child.pid;writeFileSync(resolve(out,'preview-1.json'),JSON.stringify(identity,null,2)+'\n',{flag:'wx'});
