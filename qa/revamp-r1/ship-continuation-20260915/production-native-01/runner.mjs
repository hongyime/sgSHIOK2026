...
const report={origin,deployment,requireRouteSuccess,renderer,startedAt:new Date().toISOString(),
...
try{
  ...
  check('postal input has native focus',await evaluate(`document.activeElement===document.querySelector('#postal-search-input')`));
  await send('Input.insertText',{text:'018956'});
  await until('typed postal visible',()=>evaluate(`document.querySelector('#postal-search-input').value`),v=>v==='018956',5000);
  if(buttonSubmit){
