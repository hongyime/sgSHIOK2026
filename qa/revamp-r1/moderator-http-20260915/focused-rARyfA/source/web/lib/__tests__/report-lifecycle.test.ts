...
describe('T14 proposed provider-neutral moderation, not authentication or persistence', () => {
  ...
  it('rejects NUL reasons in commands and stored decisions before a PostgreSQL write', () => {
    expect(planReportModeration(pending(), command({ reason: 'review\u0000note' }), moderator))
      .toEqual({ ok: false, error: 'invalid_command' });
    expect(planReportModeration(closed('report-a', 'accepted', { reason: 'review\u0000note' }), command(), moderator))
