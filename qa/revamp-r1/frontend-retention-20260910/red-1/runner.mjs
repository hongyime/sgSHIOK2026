...
const [label,...names]=process.argv.slice(2);if(!/^[a-z0-9-]+$/.test(label??'')||names.some(n=>!/^tests\/test_[a-z_]+\.py$/.test(n)))throw Error('Invalid receipt/target');
...
const files=['scripts/release_staging.py','pipeline/publish.py','tests/test_frontend_retention.py','tests/test_release_staging.py','tests/test_publish.py','web/next.config.js'];
