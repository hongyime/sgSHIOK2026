...
try {
  ...
  check('old module request reached actual B origin200', served.length > 0 && served.every(r => r.activeAtStart === 'B' && r.upstream === 4345 && r.status === 200), served);
  ...
  const fault=await until('future global fallback',()=>evaluate(facts),f=>f.text.includes("SHIOK couldn't load")&&f.controls.some(b=>b.label==='Reload page'));
  await shot('global-error-before-reload');
  const reloadPoint=await evaluate(()=>{
    const button=[...document.querySelectorAll('button')].find(b=>b.textContent==='Reload page');
    const box=button.getBoundingClientRect();return {x:box.x+box.width/2,y:box.y+box.height/2};
  });
  await send('Input.dispatchMouseEvent',{type:'mousePressed',button:'left',clickCount:1,...reloadPoint});
  await send('Input.dispatchMouseEvent',{type:'mouseReleased',button:'left',clickCount:1,...reloadPoint});
  const reloaded=await until('native application reload',()=>evaluate(facts),f=>f.timeOrigin!==fault.timeOrigin&&f.text.includes("SHIOK couldn't load"));
  check('global Reload preserves full query and hash',reloaded.url===faultUrl,reloaded.url);
  await shot('global-error-after-reload');
  await delay(2000);
  check('global error does not automatically loop', (await evaluate(facts)).timeOrigin===reloaded.timeOrigin);
  report.fault={url:faultUrl,before:fault.timeOrigin,after:reloaded.timeOrigin,expectedError:'QA fixture: global boundary'};
  await send('Page.navigate',{url:retained.url});
  await until('selected route after deliberate navigation',()=>evaluate(facts),ready,120000);
  await shot('final-B-selected-mobile');
  ...
  check('no unexpected application exceptions',report.errors.every(e=>e.exception?.description?.includes('QA fixture: global boundary')),report.errors);
  report.outcome = 'Retained A renders selected route from B fallback; future global Reload preserves URL';
