...
const origin = 'http://127.0.0.1:4386', hash = bytes => createHash('sha256').update(bytes).digest('hex');
...
function facts() {
  ...
  return { status: document.querySelector('main')?.dataset.mapStatus, routeKey: d?.routeKey,
    ...
    geometry: m?.getStyle().sources['shiokest-route']?.data?.features?.map(f => f.geometry),
...
try {
  ...
  await send('Page.navigate',{url:'file:///C:/sgSHIOK2026/postplan.html'},mainSession);
  await until('plan title',()=>evaluate(()=>document.querySelector('h1')?.textContent),t=>t==='SHIOK product work plan');
  const plan=await evaluate(()=>({title:document.title,viewport:[innerWidth,innerHeight],overflow:document.documentElement.scrollWidth>innerWidth,sections:[...document.querySelectorAll('h2')].map(e=>e.textContent)}));
  const planImage=Buffer.from((await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false},mainSession)).data,'base64');
  writeFileSync(resolve(out,'postplan-mobile.png'),planImage,{flag:'wx'});
  report.plan={...plan,sha256:hash(planImage),bytes:planImage.length};
  check('plan mobile fits',!plan.overflow&&plan.sections.length===4,plan);
  ...
  report.scope = 'Targeted correction: disclosure visibility after closing its ancestor, plus local plan rendering. Does not replace the preceding raw four-viewport and category run. Headless desktop, not a physical phone or speed benchmark.';
