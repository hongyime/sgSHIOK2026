...
const data=http.createServer(async(req,res)=>{
  ...
  try{
    const path=decodeURIComponent(new URL(req.url,'http://localhost:4332').pathname);
...
const proxy=http.createServer((req,res)=>{
  const path=new URL(req.url,'http://127.0.0.1:4384').pathname;
  ...
  const port=path.startsWith('/data/')?4332:4383,hostname=port===4332?'localhost':'127.0.0.1';
...
let child;
const identity={buildId:build.buildId,snapshot:build.snapshot,pid:process.pid,dataPort:4332,nextPort:4383,proxyPort:4384,readOnly:true};
try{
  await listen(data,4332,'localhost');await listen(proxy,4384,'127.0.0.1');
  child=spawn(process.execPath,[resolve(root,'web/node_modules/next/dist/bin/next'),'start',resolve(build.snapshot,'web'),'-p','4383','-H','127.0.0.1'],{cwd:root,windowsHide:true,env:{...process.env,NEXT_TELEMETRY_DISABLED:'1'},stdio:['ignore','pipe','pipe']});
