...
import { tracePage } from './trace-page.mjs';
const root = 'C:\\sgSHIOK2026';
if (process.cwd() !== root) throw Error('Working root guard');
const label = process.argv[2];
const viewports = process.argv[3] === '--viewports';
if (!/^probe-[0-9]+$/.test(label ?? '')) throw Error('Fresh probe-N required');
const out = resolve(root, 'qa/revamp-r1/map-stall-20260910', label);
if (os.freemem() < 1024 * 1048576) throw Error('Browser headroom gate: less than 1024 MiB free');
mkdirSync(out);
writeFileSync(resolve(out, 'runner.mjs'), readFileSync(new URL(import.meta.url)), { flag: 'wx' });
writeFileSync(resolve(out, 'trace-page.mjs'), readFileSync(new URL('./trace-page.mjs', import.meta.url)), { flag: 'wx' });
writeFileSync(resolve(out, 'cleanup-live.ps1'), readFileSync(new URL('./cleanup-live.ps1', import.meta.url)), { flag: 'wx' });
const profile = mkdtempSync(resolve(root, 'tmp/map-stall-browser-'));
const report = { root, host: os.hostname(), profile, startedAt: new Date().toISOString(), boundaries: [], events: [], responses: [], errors: [], captures: [], trace: [], availableMiB: os.freemem() / 1048576 };
...
function facts() {
  ...
  const features = map.queryRenderedFeatures({ layers:['shiokest-route-line'] });
  const style = map.isStyleLoaded();
  return { key:debug.routeKey, count:features.filter(f => f.properties.render_key === debug.routeKey).length,
    width:innerWidth, height:innerHeight, moving:map.isMoving(), center:map.getCenter().toArray(), zoom:map.getZoom(),
    style, basemap:style && map.isSourceLoaded('onemap'), tiles:style && map.areTilesLoaded(),
    status:document.querySelector('main')?.dataset.mapStatus, overflow:document.documentElement.scrollWidth > innerWidth,
    brand:rect(document.querySelector('h1')), search:rect(document.querySelector('#postal-search-input').closest('form')),
    sheet:rect(document.querySelector('aside')), about:rect([...document.querySelectorAll('summary')].find(n=>n.textContent==='About data')) };
}
const ready = f => f.status === 'ready' && f.count === 4 && f.style && f.basemap && f.tiles && !f.moving;
try {
  ...
  ws.onmessage = event => {
    ...
    else if (m.method === 'Runtime.bindingCalled' && m.params.name === 'qaMapTrace') { report.trace.push(JSON.parse(m.params.payload)); save(); }
  ...
  await send('Runtime.addBinding', { name: 'qaMapTrace' });
  await send('Page.addScriptToEvaluateOnNewDocument', { source: tracePage });
  if (!viewports) { await send('Profiler.enable'); await send('Profiler.start'); profiling = true; }
  ...
  // Observe application queries through the binding; do not add a QA feature query.
  await delay(60000);
  await capture('after-passive-observation-390x844');
  ...
  if (viewports) {
    ...
    for (const [width,height] of [[390,844],[1440,950],[390,667],[320,667]]) {
      await send('Emulation.setDeviceMetricsOverride', { width,height,deviceScaleFactor:1,mobile:false });
      await until(() => evaluate(`(${facts.toString()})()`, 'viewport readiness'), f=>f.width===width && f.height===height && ready(f), 60000);
      await delay(500);
      const before = await evaluate(`(${facts.toString()})()`, 'before capture');
      await capture(`loaded-${width}x${height}`);
      const after = await evaluate(`(${facts.toString()})()`, 'after capture');
      const checks = { rendered:ready(before)&&ready(after)&&before.key===after.key&&JSON.stringify(before.center)===JSON.stringify(after.center)&&before.zoom===after.zoom,
        overflow:!after.overflow, leftStack:after.brand.x<60 && after.search.x<60 && after.brand.bottom<=after.search.y && after.search.bottom<=after.sheet.y,
        equalWidth:Math.abs(after.search.width-after.sheet.width)<1 && Math.abs(after.search.x-after.sheet.x)<1,
        about:after.about.right<=width && after.about.right>=width-40 && after.about.bottom<=height && after.about.y>height-100 };
      report.viewportChecks.push({width,height,before,after,checks}); save();
      if (!Object.values(checks).every(Boolean)) throw Error('Viewport check failed');
    }
  }
  report.ok = true;
} catch (error) {
  ...
  const cleanup = spawnSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-File', resolve(root, 'qa/revamp-r1/map-stall-20260910/cleanup-live.ps1'), '-Profile', profile], { cwd: root, windowsHide: true, encoding: 'utf8', timeout: 90000 });
  ...
  writeFileSync(resolve(out, 'chrome-stderr.txt'), stderr, { flag:'wx' });
