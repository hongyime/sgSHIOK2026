...
try {
  ...
  if (mode === 'inspect') {
    ...
    const tests = ['report_moderation.sql', 'report_moderation_edges.sql']
      .map(n => readFileSync(resolve(root, 'supabase/tests', n), 'utf8')).join('\n');
    ...
    if (mode === 'apply') {
      ...
      result.isolationChecks = [];
      for (const isolation of ['repeatable read', 'serializable']) {
        const rows = await query(`begin isolation level ${isolation}; set local statement_timeout='25s';
          ${mode === 'test' ? migration : ''}
          create function pg_temp.reject_isolation() returns jsonb language plpgsql as $$
          declare code text; count integer:=0; command text;
          begin
            foreach command in array array[
              'select public.shiok_moderator_queue_v1(null,null,null,''pending'',null,null)',
              'select public.shiok_moderator_context_v1(null,null,null,null,null)',
              'select public.shiok_moderator_decide_v1(null,null,null,null,1,''accepted'',''synthetic'',null,''[]'')'
            ] loop
              code:=null;
              begin execute command; exception when others then get stacked diagnostics code=returned_sqlstate; end;
              if code is distinct from 'PT503' then raise exception 'Isolation gate did not reject'; end if;
              count:=count+1;
            end loop;
            return jsonb_build_object('isolation',current_setting('transaction_isolation'),'rejected_entry_points',count);
          end; $$;
          set local role service_role; select pg_temp.reject_isolation() proof; rollback;`, false);
        assert.equal(rows[0].proof.isolation, isolation);
        assert.equal(rows[0].proof.rejected_entry_points, 3);
        result.isolationChecks.push(rows[0].proof);
      }
    }
