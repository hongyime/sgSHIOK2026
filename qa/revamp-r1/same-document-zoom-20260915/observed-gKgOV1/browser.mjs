...
report.harnessSources=Object.fromEntries(['browser.mjs','contract.mjs','probes.mjs','native-shortcut.ps1','processes.ps1','helper-process.mjs','../lifecycle-probe-20260910/worker-session.mjs'].map(name=>[name,hash(readFileSync(resolve(BASE,name)))]));
...
try {
  ...
  ws.onmessage=event=>{
    ...
    if(m.method==='Target.attachedToTarget'&&p.waitingForDebugger){
      report.events.push({kind:'worker',target:p.targetInfo,session:p.sessionId,atMs:Date.now()-started});
      void observeWorker(send,p.sessionId).catch(error=>report.errors.push({worker:error.message}));
    }
  ...
  const unexpected=report.denied.filter(entry=>!knownBlockedBrowserTraffic(entry));
  check('no application or unaccounted remote requests',unexpected.length===0&&report.errors.length===0,{unexpected,blockedBrowserTraffic:report.knownBlockedBrowserTraffic,errors:report.errors});
  report.passed=true;
