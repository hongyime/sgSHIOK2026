...
import { ROOT, BASE, METRICS, BUDGET, config, ownedProfile, sameSelection, nativeZoom, allMetrics, transition } from './contract.mjs';
...
function powershell(file,args=[],ms=15000) {
  ...
  const stdout=execFileSync('powershell.exe',['-NoProfile','-NonInteractive','-File',resolve(BASE,file),...args],{cwd:ROOT,windowsHide:true,env,encoding:'utf8',timeout:Math.min(ms,remaining),maxBuffer:1024*1024});
...
try {
  ...
  report.memory=JSON.parse(execFileSync('powershell.exe',['-NoProfile','-NonInteractive','-Command','Get-CimInstance Win32_OperatingSystem | Select-Object FreePhysicalMemory | ConvertTo-Json -Compress'],{cwd:ROOT,env,windowsHide:true,encoding:'utf8',timeout:10000}));
