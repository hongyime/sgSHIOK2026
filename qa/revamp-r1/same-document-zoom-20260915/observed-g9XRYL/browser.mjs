...
import { ROOT, BASE, POWERSHELL, METRICS, BUDGET, config, ownedProfile, sameSelection, nativeZoom, allMetrics, transition, knownBlockedBrowserTraffic } from './contract.mjs';
...
const report={root:ROOT,hostname:process.env.COMPUTERNAME,out,profile,startedAt:new Date(started).toISOString(),target,
  scope:'Same-document native Windows Chrome shortcuts100->200->100%, keyboard map and complete metric reading. No CSS/page-scale/device-metric emulation. Exact local candidate; optional bounded public OneMap display-image capture only. No pipeline, remote API, physical-phone, screen-reader or performance claim.',
...
report.harnessSources=Object.fromEntries(['browser.mjs','contract.mjs','probes.mjs','native-shortcut.ps1','processes.ps1','helper-process.mjs','display-images.mjs','../lifecycle-probe-20260910/worker-session.mjs'].map(name=>[name,hash(readFileSync(resolve(BASE,name)))]));
...
async function powershellArgs(args,ms=15000) {
  const remaining=(closing?deadline:workEnd)-Date.now();assert.ok(remaining>0,'Native helper deadline');
  const step={kind:'helper',args,startedMs:Date.now()-started};report.events.push(step);
  try{
    const result=await helperProcess(POWERSHELL,['-NoProfile','-NonInteractive',...args],{cwd:ROOT,env,timeout:Math.min(ms,remaining)});
    step.stdout=result.stdout;step.stderr=result.stderr;
    return JSON.parse(result.stdout.replace(/^\uFEFF/,''));
  }catch(error){step.error=error.message;step.stdout=error.stdout;step.stderr=error.stderr;step.killed=error.killed;throw error;}
  finally{step.elapsedMs=Date.now()-started-step.startedMs;}
}
async function powershell(file,args=[],ms=15000){return powershellArgs(['-File',resolve(BASE,file),...args],ms);}
async function native(action) {
  const value=await powershell('native-shortcut.ps1',['-BrowserPid',String(chrome.pid),'-Profile',profile,'-Action',action,'-Trace']);
...
const display=target.captureDisplayImages===true?displayImages({end:workEnd,save:(name,bytes)=>write(name,bytes)}):null;
try {
  report.sourcesBefore=sourceHashes();
  report.memory=await powershellArgs(['-Command','Get-CimInstance Win32_OperatingSystem | Select-Object FreePhysicalMemory | ConvertTo-Json -Compress'],10000);
  ...
  proxy=createServer((req,res)=>{req.resume();report.denied.push({kind:'proxy',url:req.url,method:req.method});res.writeHead(403).end();});
  ...
  ws.onmessage=event=>{
    ...
    if(m.method==='Log.entryAdded')report.events.push({kind:'browser-log',entry:p.entry,atMs:Date.now()-started});
    if(m.method==='Network.loadingFailed')report.events.push({kind:'loading-failed',details:p,session:sid,atMs:Date.now()-started});
    ...
    if(m.method==='Target.attachedToTarget'){
      report.events.push({kind:'worker',target:p.targetInfo,session:p.sessionId,atMs:Date.now()-started});
      if(p.targetInfo.type==='worker')void observeWorker(send,p.sessionId).catch(error=>report.errors.push({worker:error.message,target:p.targetInfo,session:p.sessionId}));
    }
    if(m.method==='Fetch.requestPaused')void (async()=>{
      ...
      if(!display&&replies.has(url.href)&&method==='GET') {
      ...
      if(display&&method==='GET'&&displayImageUrl(url.href)) {
        const bytes=await display.get(url.href);
        const receipt=display.receipts.find(entry=>entry.url===url.href);
        await send('Fetch.fulfillRequest',{requestId:p.requestId,responseCode:200,responseHeaders:[{name:'Content-Type',value:receipt.contentType},{name:'Access-Control-Allow-Origin',value:receipt.accessControlAllowOrigin}],body:bytes.toString('base64')},sid);return;
      }
      ...
      report.errors.push({interception:error.message,url:p.request.url});
      try{await send('Fetch.failRequest',{requestId:p.requestId,errorReason:'BlockedByClient'},sid);}catch(failError){report.errors.push({interceptionClose:failError.message});}
    });
  };
  ...
  report.initialNative=await native('reset');
  ...
  const baseline=await evaluate(probes.facts),windowBaseline=await native('inspect');
  ...
  const doubled=await evaluate(probes.facts),windowDoubled=await native('inspect');
  ...
  await settled('100 percent restored route');const restored=await evaluate(probes.facts),windowRestored=await native('inspect');
  ...
  report.knownBlockedBrowserTraffic=report.denied.filter(knownBlockedBrowserTraffic);
  const unexpected=report.denied.filter(entry=>entry.kind==='page');
  report.unattributedBlockedProxyTraffic=report.denied.filter(entry=>entry.kind!=='page'&&!knownBlockedBrowserTraffic(entry));
  check('no forbidden application requests',unexpected.length===0&&report.errors.length===0,{unexpected,blockedBrowserTraffic:report.knownBlockedBrowserTraffic,unattributedBlockedProxyTraffic:report.unattributedBlockedProxyTraffic,errors:report.errors});
  report.passed=true;
} catch(error) {
  report.failure=error.stack;console.error(error.stack);
  if(ws?.readyState===1&&session)try{
    const shot=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false},session,5000);
    const bytes=Buffer.from(shot.data,'base64');write('failure.png',bytes);
    report.failureCapture={file:'failure.png',sha256:hash(bytes),bytes:bytes.length,facts:await evaluate(probes.facts)};
  }catch(captureError){report.failureCaptureError=captureError.message;}
}
finally {
  closing=true;
  if(display){await display.drain();report.displayImages={...display.stats(),receipts:display.receipts};}
  report.passed=report.passed&&report.errors.length===0&&!report.denied.some(entry=>entry.kind==='page');
  if(chrome?.pid) {
    try {
      const owned=await powershell('processes.ps1',['-BrowserPid',String(chrome.pid),'-Profile',profile,'-Phase','snapshot'],8000);
      write('owned-before-close.json',JSON.stringify(owned,null,2)+'\n');
    }catch(error){report.snapshotError=error.stack;}
    ...
    try{
      report.cleanup=await powershell('processes.ps1',['-BrowserPid',String(chrome.pid),'-Profile',profile,'-Phase','cleanup','-Receipt',resolve(out,'owned-before-close.json')],35000);
  ...
  report.passed=report.passed&&report.errors.length===0&&!report.denied.some(entry=>entry.kind==='page')&&report.cleanup.verified&&report.sourcesUnchanged&&!report.proxyCleanupError&&report.elapsedMs<=BUDGET.total;
