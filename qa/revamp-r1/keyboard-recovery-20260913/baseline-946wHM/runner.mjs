...
try {
  ...
  function focus(){const e=document.activeElement,r=e?.getBoundingClientRect();return {tag:e?.tagName,label:e?.getAttribute('aria-label'),text:e?.textContent?.trim(),id:e?.id,
  ...
  await send('Page.navigate',{url:origin+'/?debugMap=1&postal=018956&transit=bus&stop='+encodeURIComponent(stop)},mainSession);
