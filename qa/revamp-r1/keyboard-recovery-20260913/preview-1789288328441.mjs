...
const out=resolve(root,'qa/revamp-r1/keyboard-recovery-20260913');
...
const data=http.createServer(async(req,res)=>{
  ...
  try{
    const path=decodeURIComponent(new URL(req.url,'http://localhost:4338').pathname);
...
const proxy=http.createServer((req,res)=>{
  const path=new URL(req.url,'http://127.0.0.1:4414').pathname;
  ...
  const port=path.startsWith('/data/')?4338:4413,hostname=port===4338?'localhost':'127.0.0.1';
...
let child;
const identity={buildId:build.buildId,snapshot:build.snapshot,pid:process.pid,dataPort:4338,nextPort:4413,proxyPort:4414,readOnlyData:true, allowedApi:'GET /api/onemap-route and /api/onemap-search', oneMapCredentialsConfigured:Boolean(credentials.ONEMAP_EMAIL && credentials.ONEMAP_PASSWORD)};
try{
  await listen(data,4338,'localhost');await listen(proxy,4414,'127.0.0.1');
  child=spawn(process.execPath,[resolve(root,'web/node_modules/next/dist/bin/next'),'start',resolve(build.snapshot,'web'),'-p','4413','-H','127.0.0.1'],{cwd:root,windowsHide:true,env:{...process.env,...credentials,NEXT_TELEMETRY_DISABLED:'1'},stdio:['ignore','pipe','pipe']});
