...
const files=paths.filter(p=>p!==prefix+'index-check.json').map(path=>({path,diskSha256:hash(readFileSync(resolve(root,path))),indexSha256:hash(git(['show',':'+path]))}));
const audit=JSON.parse(readFileSync(resolve(root,prefix+'audit.json')));
for(const source of audit.sourceIdentity)if(hash(git(['show',':'+source.path]))!==source.sha256)throw Error('Indexed source not tested: '+source.path);
...
const result={base,evidencePrefix:{bytes:prior.length,sha256:hash(prior),diskUnchanged:true,indexUnchanged:true},files,commands,
...
writeFileSync(resolve(root,prefix+'index-check.json'),JSON.stringify(result,null,2)+'\n',{flag:'wx'});
console.log(JSON.stringify({files:files.length,normalizedFiles:files.filter(f=>f.diskSha256!==f.indexSha256).map(f=>f.path),evidencePrefix:result.evidencePrefix,commands},null,2));
