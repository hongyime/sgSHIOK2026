...
try {
  ...
  check('actual local load fired with remote tiles held',report.mapEventsBeforeRelease.some(e=>e.event==='load')&&!report.mapEventsBeforeRelease.some(e=>e.event==='map-ref-cleared'),report.mapEventsBeforeRelease);
  holdTiles=false;
  report.releases=[];
