...
const out = mkdtempSync(resolve(directory, 'baseline-'));
...
const report = { root, hostname: process.env.COMPUTERNAME, out, profile, origin,
  startedAt: new Date().toISOString(), setup: 'Fresh B worker-controlled profile. Hold only actual OneMap raster requests; no byte substitution or app-state mutation. Record map lifecycle before the startup watchdog.',
...
try {
  ...
  ws.onmessage = event => {
    ...
    if (m.id) pending.get(m.id)?.finish(m.result, m.error);
      ...
      if (request.method==='GET' && u.origin==='https://www.onemap.gov.sg' && /^\/maps\/tiles\/Grey_HD\/\d+\/\d+\/\d+\.png$/.test(u.pathname)) {
  ...
  const instrument=()=>{
    ...
    const facts=(map,event,extra={})=>{
      ...
      try{routeFeatures=map.getSource('shiokest-route')?._data?.features?.length??null;}catch{}
  ...
  await shot('baseline-pending-tiles');
  const outcome=await until('baseline startup terminal',()=>evaluate(facts),f=>f.status==='error',60000);
  report.mapEvents=await evaluate(()=>window.__qaMapStartup.events);
  report.failure=outcome;
  await shot('baseline-startup-timeout');
  await documents();
  check('actual current B document',report.documents.some(d=>d.containsB&&!d.containsA));
  check('only basemap transport deliberately held',report.heldTiles.length>0&&report.denied.length===0,{held:report.heldTiles.length,denied:report.denied});
  check('style initialized before tile-dependent load',report.mapEvents.some(e=>e.event==='style.load')&&!report.mapEvents.some(e=>e.event==='load'),report.mapEvents);
  check('walk text remains but selected route was never submitted',outcome.text.includes('55%')&&outcome.text.includes('Bayfront')&&report.mapEvents.filter(e=>e.event==='render').every(e=>e.routeFeatures===0),report.mapEvents.filter(e=>e.event==='render'));
  check('baseline disposes renderer at startup deadline',outcome.featureCount===0&&outcome.text.includes('The map did not finish starting')&&report.mapEvents.some(e=>e.event==='map-ref-cleared'),outcome);
  report.outcome='Controlled reproduction: pending OneMap rasters prevent initial load, route submission and map survival';
