...
const directory=resolve(root,'qa/revamp-r1/basemap-startup-20260910',label);
...
const live=await fetch('http://127.0.0.1:4349/',{signal:AbortSignal.timeout(30000)});
if(!live.ok||!(await live.text()).includes(build.buildId))throw Error('Wrong actual Next build');
const identity={root,directory,pid:process.pid,origin:'http://127.0.0.1:4350',buildB:build.buildId,buildA:'ZHqFMae_a9ROgk43z7fv8',sourceB:build.snapshot,active:'B'};
...
const server=http.createServer((req,res)=>{
  ...
  if(url.pathname==='/__qa/seed'){res.writeHead(200,{'Content-Type':'text/html','Cache-Control':'no-store'}).end('<!doctype html><title>Retained-tab fixture seed</title>Local fixture seed.');return;}
  const port=url.pathname.startsWith('/data/')?4321:4349,hostname=port===4321?'localhost':'127.0.0.1';
