...
const directory=resolve(root,'qa/revamp-r1/basemap-startup-20260910',attempt);
...
const live=await fetch('http://127.0.0.1:4347'+target,{signal:AbortSignal.timeout(15000)});
...
const identity={root,directory,pid:process.pid,origin:'http://127.0.0.1:4348',buildA,buildB:b.buildId,workerHashes,target,targetBytes:targetBytes.length,targetSha256:sha(targetBytes),sourceB:b.snapshot};
...
const server=http.createServer((req,res)=>{
  ...
  const data=url.pathname.startsWith('/data/'),port=data||active==='A'?4321:4347,hostname=port===4321?'localhost':'127.0.0.1';
...
server.listen(4348,'127.0.0.1',()=>{writeFileSync(resolve(directory,'runner.mjs'),readFileSync(new URL(import.meta.url)),{flag:'wx'});writeFileSync(receipt,JSON.stringify(identity,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(identity));});
