...
Records with shelter-map evidence but missing subscores now show `Locked score inputs unavailable` instead of `Locked terms unavailable`.
...
npm notice run node scripts/test-web.mjs lib/__tests__/accessibility-render.test.tsx lib/__tests__/score-card-copy.test.ts
 ...
 Test Files  2 passed (2)
      Tests  55 passed (55)
   Start at  12:09:15
   Duration  18.09s (transform 5.80s, setup 0ms, import 6.98s, tests 4.43s, environment 2ms)
```
...
```text
git check-ignore -v qa/verification/P654-locked-score-inputs-copy.md; Write-Output "exit=$LASTEXITCODE"
```
...
npm notice run node scripts/test-web.mjs
 ...
 Test Files  24 passed (24)
   ...
   Start at  12:10:06
   Duration  47.51s (transform 2.60s, setup 0ms, import 4.76s, tests 20.09s, environment 11ms)
```
...
1. The reason-chip copy still used internal `Locked terms unavailable` wording. `Locked score inputs unavailable` is clearer for users while preserving the same missing-data state.
