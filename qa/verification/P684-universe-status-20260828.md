# P684 Universe Status, 2026-08-28
...
```text
PS C:\sgSHIOK2026> git check-ignore -v qa/verification/P684-universe-status-20260828.md; Write-Output "exit=$LASTEXITCODE"
exit=1
```
...
```text
PS C:\sgSHIOK2026> uv run python run.py universe-status; Write-Output "exit=$LASTEXITCODE"
{
...
1. The current cached universe-status report still sizes the confirmed recent-public-source frozen-v1 gap at 6 of 976 rows with postals, or 0.614754%, with a directional scale estimate of 765 confirmed missing address rows if that sample row rate were applied to 124,443 frozen-v1 postals.
2. Including MCST source-quality warnings raises the cached sample signal to 8 of 976 rows, or 0.819672%, with a directional scale estimate of 1,020 rows. The evidence continues to distinguish the 6 coordinate-backed HDB gaps from the 2 MCST proxy warnings.
3. The cached P125 Overpass cross-check remains a coverage check, not an address registry: 25,873 valid OSM postcodes overlap frozen v1, 6 valid OSM postcodes are not in v1, and OSM covers 20.791045% of v1.
