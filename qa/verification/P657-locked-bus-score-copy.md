...
Direct-bus fallback copy now says the `locked bus score remains 0` instead of saying the `bus term remains 0`.
...
npm notice run node scripts/test-web.mjs lib/__tests__/accessibility-render.test.tsx lib/__tests__/score-card-copy.test.ts
 ...
 Test Files  2 passed (2)
      Tests  55 passed (55)
   Start at  12:28:49
   Duration  5.91s (transform 1.92s, setup 0ms, import 2.39s, tests 1.54s, environment 1ms)
```
...
```text
git check-ignore -v qa/verification/P657-locked-bus-score-copy.md; Write-Output "exit=$LASTEXITCODE"
```
...
npm notice run node scripts/test-web.mjs
 ...
 Test Files  24 passed (24)
   ...
   Start at  12:29:22
   Duration  47.49s (transform 2.90s, setup 0ms, import 5.95s, tests 12.98s, environment 22ms)
```
...
1. Two direct-bus fallback sentences still used `bus term remains 0`. `Locked bus score remains 0` is clearer to users and aligns with the visible bus score row.
