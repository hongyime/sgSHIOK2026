...
Safe reports:
  ...
  p19-gap-status reads cached P19 measurement status, missing rows, MCST proxy probe and cache ages only; it calls no APIs and writes no files.
...
STUBS = {
    ...
    "p19-gap-status": "read-only status, missing rows, MCST proxy probe and cache ages for cached P19 postal-universe gap measurement",
