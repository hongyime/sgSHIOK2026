...
- Retained-current acceptance2026-09-12: controlled local M17 sequence now passes27checks/7inspected captures with current156-source build uvi18B-a0b6Sit2hVCydw. Old A retains exact Document/postal after B worker activation and first uncached old-module fetch, real fallback200/39556byte hash verified,4features at4sizes. Ordinary navigation renders B desktop/mobile;4cached data samples and2sentinels unchanged. First15pass/1transitional resize-capture failure preserved;11contracts validate corrected3sample/bounded screenshot barrier. QA servers/browser stopped. This closes the previously failing retained-A plus current-B sequence; real release identity, old-runtime security, other-platform scope remain separate, not new permission to deploy. No repeat of this local sequence merely for PASS. Evidence: qa/revamp-r1/retained-current-20260912/summary.json.
- Depends on: none.
...
- Retained-tab follow-up 2026-09-10: M17 is now a demonstrated FAIL in a controlled local legacy-A/current-B rehearsal, not an untested assumption. An exact never-requested/cached A map chunk first reaches B after worker activation and returns real404 through the SW. A loses its walk view to the global fallback; that fallback's empty GET Reload drops the postal. Final browser18pass+2fail=20checks;7captures parent-inspected. Separate native re-search restores4features at4sizes, not preserved-selection recovery. Both sentinels and4sampled cached data bodies survive.102focused tests/3files and integrity pass;144websource/11anchor identities unchanged. No app edits, build, install, pipeline or deploy. Evidence: `qa/revamp-r1/retained-tab-20260910/summary.json`; prior setup/partial failures preserved.
- Next FREE implementation: design fixture-tested retention of verified previous immutable frontend chunks in release preparation, rejecting same-path/different-byte conflicts and bounding retained generations/bytes. Identify the actual previous release before any real staging. A future global fallback should preserve postal/share URL on Reload, but cannot repair already-loaded legacy code. Do not force worker-driven navigation that could discard report drafts. Keep T01/T27 gated until the retained-tab case is accepted; local A is not newly proven production identity. No repeated browser run merely to seek a pass.
- Retention implementation 2026-09-10: pinned archive capture/staging, fallback-only old URLs and pre/post-build conflict guards landed with166 targeted Python tests and1744/64 isolated web tests, TypeScript/build/integrity.29 captured runtime files=5786770bytes; no data copied. Retained A keeps the exact Document/postal and4features at4sizes, but the later B navigation hits map-startup timeout: browserexit1,15 checks passed before timeout, not fullM17. Separate global-error fixture passes7checks and preserves query/hash on native Reload.9captures parent-inspected; peer quota prevents independent acceptance. All failed attempts retained. T01 stays PARTIAL; next free diagnostic needs current-B worker/style/tile lifecycle and failure capture, not blind repetition. Real old-release identity/security, T25 and T27/T28 remain gated. Evidence: `qa/revamp-r1/frontend-retention-20260910/summary.json`.
- Basemap follow-up 2026-09-10: controlled delayed OneMap requests exposed a real dependency bug: initial MapLibre load waited for rasters, and route submission waited for load. The renderer now starts with local background/worker sources before attaching the unchanged basemap below all overlays. Its separate30-second tile deadline keeps the map/route; recovery clears only its own timeout. Worker/generic/selected-route failures retain their owners.93focused and1755/64 isolated web tests, TypeScript/build/integrity pass;149sources match tested/built bytes and11anchors match. Corrected browser replay passes14checks/9inspected screenshots: route before tiles, timeout survival, actual tile recovery without document/selection reset,4viewports, ordinary navigation and a real held-worker failure. Initial baseline/treatment harness assertions remain recorded as failures, not passes;14images inspected in total. T01 current-build startup slice accepted by parent, not independent review or representative speed measurement. Retained-A was not replayed with this change; real-release identity/security/T25/T27/T28 remain gated. Evidence: `qa/revamp-r1/basemap-startup-20260910/summary.json`.

### [ ] T02: Reduce the next measured loading bottleneck
- Status: PARTIAL. Size: M. Parent: P0.3.
- Settled-geometry fix2026-09-12: first selection includes already-completed geometry; identical final geometry no longer republishes primary. New test failed2updates vs1 before fix.1788/65 isolated tests+42guards,types/integrity/build pass.3new cases ready/missing/rejected; existing pending/stale cases pass. Built mobile cold/warm3checks/2inspected captures4features; warm route source receives1populated write versus2previously. No latency improvement claim under measured near100%CPU/paging; stop repeated profiles. Preview4374serves new build2aYxZG6C5VyvZ5pQfNOkD. Evidence: qa/revamp-r1/settled-geometry-20260912/summary.json. Representative/M12 acceptance remains open.
...
- Normalization follow-up2026-09-10: exact segment/fragment matching now indexes candidate start coordinates once per validated part set, retaining reverse matching, repeated positions and strict part boundaries.37 native checks pass, including29282 comparisons against the original matcher and20 full normalizer/coverage differential inputs. Unique-start1024vertex fixture drops1051642 coordinate reads to6140; synthetic timings are not population/browser evidence. All9 profiled outputs match; retained fixtures omit route_segments, so synthetic cases are separately labelled. Installed TypeScript/integrity pass;8 new Vitest cases are typechecked but not run under mismatched dependencies. The scanner now pins compile-4 with5sources/outputs; old pilots cannot authorize it. No data scan/new900s projection/app build/deploy. Evidence: `qa/revamp-r1/normalization-profile-20260910/summary.json`.
...
- Follow-up 2026-09-10, current motion replay: fixed three stale-ready races before passive-effect cleanup and keyboard focus hidden beneath the sticky comparison label. Full isolated suite1765/64 (1755+3+7), TypeScript/build/integrity pass;149 current/tested/built sources and11anchors match. Final browser52/53checks and14captures prove functional navigation, four viewports, text doubling/reflow and18 zero-duration route fits, but its final audit fails on5 CDP interception faults. It remains exit1, not a clean browser PASS. All original failures and43image files are preserved; parent review only. Evidence: `qa/revamp-r1/cross-feature-motion-20260910/summary.json`.
- Next: correlate interception command faults with request IDs and Network cancellation before one bounded clean audit; do not silently ignore them or rerun merely to obtain PASS. Retained-old-tab M17 remains open. Native zoom, assistive technology, physical devices and representative performance remain separate. Reporting stays excluded until T18. T25 remains PARTIAL.
...
- Approved alignment2026-09-10: npm ci with lifecycle scripts disabled preserved the lock; installed MapLibre6.4.1/Next16.3.3/Vitest4.1.11 match it. Worker/shared/license were copied byte-for-byte into a new6.4.1 directory;6.1.0 remains intact. Both namespaces retain immutable headers and cache behavior. Twelve targeted failures reproduced before wiring;1785web tests/65files plus42native guard contracts pass with a documented15s test timeout. The first full run's1784pass/1five-second timeout remains recorded. npm audit reports0known vulnerabilities. Build sou6pZfEMMmsCl52vdXg4 matches156current sources, uses no protected payload copy, and is available on4362. Do not infer current map rendering or latency from these checks. Evidence: `qa/revamp-r1/worker-alignment-20260910/summary.json`; follow-up `cleanup-final/` verifies no owned browser processes remain.
- Prevention progress 2026-09-10: direct installed versions must match the lock before test/package-build/release-CLI/QA-snapshot work.42 native Node contracts pass, including malformed metadata and refusal before preparation doubles. Real entry points correctly refuse MapLibre6.4.1/6.1.0, Next16.3.3/16.3.0 and Vitest4.1.11/4.1.10; no full-suite/build success under this lock is claimed. Installed TypeScript and integrity pass. Guard is not a transitive, integrity or worker audit. Installation/worker alignment remains OWNER. Evidence: `qa/revamp-r1/dependency-alignment-20260910/summary.json`.
...
- Gate: dependency installation approval fulfilled; current visual/retained-client browser acceptance remains open. Zero pipeline and no deployment included. Other independent free frontend tasks may proceed.
- Pre-approval evidence remains at qa/revamp-r1/security-triage-20260909/source-corrected.json, including the earlier guessed-path rg failure in checks.json. The later owner-approved installation is recorded separately above.
