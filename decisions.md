...
2026-09-06 - Product revamp architecture accepted:
The owner approved postal-to-nearby-transit first, with arbitrary origin/destination later; no live weather; a small useful transit candidate list exposing distance/shelter trade-offs; and separate mapping corrections and shelter suggestions, private pending owner moderation. The service remains free and non-commercial with a strict $0 budget, scheduled source checks and periodic validated releases. ARCHITECTURE.md, ARCHITECTURE-DECISIONS.md, PRODUCT-PLAN.md and qa/SHIOK-acceptance-tests.md define the implementation contract. The test catalogue is planned coverage, not executed tests. Feedback infrastructure, compute runs and production deployment retain explicit approval gates. UI mockups are the next reviewable deliverable; existing values and locked weights are unchanged.
...
2026-09-07 — Round 1 review repairs: MapLibre 6.1.0 resolves its default module worker relative to the bundled main module, producing a Next.js 404 and leaving route sources unloaded while raster tiles appear. Serve the exact installed worker/shared module and BSD license at a versioned same-origin static path, verified by hashes against the installed package. Preserve CSP and pinned dependencies. Geometry 404 misses remain cached; network/server failures propagate and are retryable. Measure actual DOM overlays and bind readiness to the current rendered route generation. Acceptance remains pending until browser races, failure recovery and cold/warm profiling finish.
...
2026-09-18 - Postal-universe v2 recurring 72h retry loop (owner-approved, executed):

Owner explicitly approved a recurring loop ("keep polling... hit the 72 hours period again and again and no downtime", "machine will definitely be on") to keep chipping at the remaining 475 NEEDS_GEOCODE postals across successive OneMap token windows, with a mandatory self-stop and self-cleanup rule. Corrected an earlier scoping error before building: `pipeline.geocode_universe.py` searches OneMap by 6-digit postal code only (`searchVal: postal`), not by address text -- there is no "better candidate text to feed OneMap" mechanism. What Overture Maps actually contributes is direct candidate coordinates for postals it has SG address rows for, bypassing OneMap entirely for those matches; `pipeline.overture_addresses.py` itself explicitly documents Overture as "candidate only until raw archive, hash/provenance, dedupe, coordinate validation, and attribution review pass" -- so Overture matches are logged for owner review, never auto-promoted to READY_TO_SCORE the way an OneMap-confirmed SUCCESS is.

Built `tmp/geocode_v2_cycle.py` (one cycle per invocation) plus `tmp/geocode_v2_cycle_run.bat` (working-directory/env wrapper) and registered Windows Task Scheduler task `SHIOK-GeocodeV2-Cycle` (`schtasks /create ... /sc DAILY /mo 3 /st 19:15 /sd 21/09/2026`, next run 21 Sep 2026 19:15 SGT, exactly 72h after today's OneMap window). Each cycle: (1) checks Overture SG address data against the current remaining NEEDS_GEOCODE list at zero OneMap cost, logging matches to `qa/revamp-r1/geocode-v2-loop-20260918/overture-candidates/` for review only; (2) runs a bounded `geocode_universe_gaps()` OneMap retry capped at the documented 250-call limit, auto-promoting genuine SUCCESS results into a freshly versioned universe parquet exactly as the manual run did; (3) never touches frozen v1 or the separately-running full-batch rescore's inputs/outputs. State (cycle count, latest universe path, consecutive-zero-progress counter, consecutive-error counter, full history) persists in `qa/revamp-r1/geocode-v2-loop-20260918/state.json`.

Stop conditions, each of which makes the cycle script delete its own scheduled task via `schtasks /delete /tn SHIOK-GeocodeV2-Cycle /f` before exiting: (a) NEEDS_GEOCODE reaches 0; (b) two consecutive cycles find zero new OneMap successes AND zero new Overture matches (free sources exhausted for now); (c) a hard cap of 10 cycles (~30 days) regardless; (d) two consecutive cycle errors. The owner can also force-remove the task manually at any time with the same `schtasks /delete` command, independent of the script's own logic.

Verified before registering: state.json seeded with cycle=2 and a history entry for the already-completed manual cycle 1 (476->475, 250 HTTP requests, 1 SUCCESS, run outside this script directly via `pipeline.geocode_universe`); the Overture cross-reference function was test-invoked standalone (zero OneMap cost) and found 1 real candidate coordinate match among the 475 already, logged as evidence only, not promoted. This does not score, export, publish, mutate frozen v1, or touch the running rescore's inputs/outputs; it does not alter locked weights.

2026-09-18 - Postal-universe v2 bounded geocode retry (owner-approved, executed):

Owner explicitly approved and directed immediate execution ("i give green light... complete phase 1 and phase 2... auto promote") of a bounded retry against the 476 `NEEDS_GEOCODE` postals in frozen v1 (confirmed via `tmp/batch-plan-20260916.json` v1_universe.status_counts). Used the existing `pipeline.geocode_universe` tool (never repairs frozen v1 in place; requires explicitly versioned output). Ran dry-run first (`--dry-run`, 0 HTTP requests, confirmed queued_postals=476), then a real bounded run: `uv run python -m pipeline.geocode_universe --input processed\postal_universe_candidate_full_registered_geocoded.parquet --output processed\postal_universe_candidate_full_registered_geocoded_v2.parquet --summary processed\postal_universe_candidate_full_registered_geocoded_v2_summary.json --db raw\geocode_cache_v2.db --limit 250 --retry-cached-failures --confirm-bounded-geocode`. Respected the documented 250-call token-authenticated OneMap cap (decisions.md line 621) exactly: 250 HTTP requests made, 1 SUCCESS, 249 confirmed still NOT_FOUND on retry with unchanged address text. Result: needs_geocode_after=475, ready_to_score_after=123968 (was 123967). New versioned universe written to `processed/postal_universe_candidate_full_registered_geocoded_v2.parquet` (124,443 rows, gitignored local artifact, not committed) and a fresh versioned geocode cache `raw/geocode_cache_v2.db`; the original frozen `processed/postal_universe_candidate_full_registered_geocoded.parquet` was not touched (confirmed by distinct output path). This is the "promotion" the owner approved: a new candidate-universe file with the 1 newly-resolved postal marked ready-to-score, not a scoring/export/publish action.

1/250 (0.4%) resolving matches the low hit-rate expectation set by prior evidence (P379/§1131: 2/2 similar retries either mismatched to a different postal or stayed NOT_FOUND). Overture-sourced candidate evidence (`pipeline.overture_addresses`, a genuinely new-candidate-text source rather than a bare retry) was considered for the remaining 475 but deferred: no local Overture archive exists yet, a fresh pull would require a new remote download competing for disk/network/CPU with the currently-running full-batch rescore (PID 72436, Stage 2, started 2026-09-17 23:25:44), and any newly-sourced candidates still could not be OneMap-validated this session regardless, since the 250-call window is now exhausted. The remaining 226 never-attempted plus 249 confirmed-NOT_FOUND (475 total) need a second bounded batch after the documented 72-hour token refresh, optionally combined with Overture-sourced candidate text at that time for a better hit rate than a bare retry. This does not score, export, publish, mutate frozen v1, or touch the running rescore's inputs/outputs; it does not alter locked weights.

2026-09-19 - Correction: OneMap "250-call window" assumption was wrong; postal-universe v2 OneMap-retry lever now exhausted:

Owner directly challenged the 2026-09-18 entries' assumption ("we sure onemap window exhausted??"). Verified against the raw cached OneMap documentation text itself (`qa/verification/P203-onemap-candidate-controls.md` line 25: "Users onboard the token-based authentication will have a higher API call limit capped at 250") and found two errors in the prior entries: (a) that sentence names token-AUTHENTICATED usage, but `pipeline.geocode_universe.fetch_onemap_postal` never sends an Authorization header or token -- confirmed by reading its source and by a live probe (`GET /api/common/elastic/search?searchVal=018956...`) that returned HTTP 200 with correct real results despite an inline `"error": "Authentication token missing"` field, i.e. the public/unauthenticated tier works fine and is what this tool actually uses; (b) no time window (daily/72h/per-token) was ever stated for that 250 figure in the source documentation at all -- the 72-hour figure only describes token validity/refresh, an unrelated fact the prior entry incorrectly conflated with the call-cap. The actually-tested, documented constraint in this codebase is sustained request RATE (`pipeline/config/params.yaml` `onemap.highest_sustained_clean_rate_rps: 1.0`, halved to `derived_client_throttle_rps: 0.5` for safety margin, from a real empirical ladder probe, `logs/onemap_probe.csv`), not a total-count-per-window cap.

Ran the remaining batch same day rather than waiting until the originally-scheduled 21 September: `uv run python -m pipeline.geocode_universe --input processed\postal_universe_candidate_full_registered_geocoded_v2.parquet --output processed\postal_universe_candidate_full_registered_geocoded_v3.parquet --summary processed\postal_universe_candidate_full_registered_geocoded_v3_summary.json --db raw\geocode_cache_v2.db --limit 475 --confirm-bounded-geocode` (reusing yesterday's cache DB, `--retry-cached-failures` intentionally omitted so the 249 already-confirmed NOT_FOUND postals skipped instantly at zero HTTP cost; only the 226 genuinely-untried postals spent real requests). Result: 226 HTTP requests, 0 new SUCCESS, needs_geocode_after=475 unchanged. Milestone: all 476 original NEEDS_GEOCODE postals have now been queried against OneMap's live search index at least once -- 1 total SUCCESS (from the 2026-09-18 run), 475 confirmed NOT_FOUND. Retrying the same unchanged 6-digit postal-code query against OneMap has 0% further expected yield; this lever is exhausted for these specific postals regardless of how much time passes, since nothing about the query or (most likely) OneMap's own index for these old records is expected to change.

Fixed `tmp/geocode_v2_cycle.py`'s OneMap step to `retry_cached_failures=False` (was `True`) so the still-running `SHIOK-GeocodeV2-Cycle` scheduled task (2026-09-18 loop entry) does not waste future cycles re-asking confirmed-dead queries; each future cycle's OneMap step will now correctly no-op (near-instant cache skip) unless the input universe itself gains new NEEDS_GEOCODE rows, which it will not on its own. Re-ran the Overture cross-reference (zero OneMap cost, unaffected by any of the above) and confirmed the same single candidate persists: postal 756935, source `OpenAddresses/Singapore Land Authority` (via Overture), lon 103.81298 lat 1.46010, address_rows=1 with a ~1.5m bounding-box spread (a precise single-point address, not an averaged cluster) -- logged to `qa/revamp-r1/geocode-v2-loop-20260918/overture-candidates/cycle_02_overture_candidates.json` for owner review, still not auto-promoted per the tool's own validation caveat. State updated in `qa/revamp-r1/geocode-v2-loop-20260918/state.json` (cycle 3 next, consecutive_zero_progress=0 since Overture found something this cycle even though OneMap did not). The scheduled task itself is unchanged (still fires 21 Sep 19:15 SGT next) and its only remaining real function going forward is the recurring free Overture check -- which is correct, since that is genuinely the only lever with any remaining expected value for these 475 postals. This does not score, export, publish, mutate frozen v1, or touch the running rescore's inputs/outputs; it does not alter locked weights.

2026-09-19 - Scheduler deleted, postal 756935 promoted, separate bigger finding flagged (owner-approved, executed):

Owner correctly questioned the scheduled task's continued value ("confused why do we still need scheduler bro?"). Root cause: `pipeline.overture_addresses.DEFAULT_OVERTURE_PATH` is a hardcoded, pinned S3 release path (`s3://overturemaps-us-west-2/release/2026-07-22.0/theme=addresses/type=address/*`), not a live feed -- re-querying it every 72h returns the identical result forever, no new discovery happens without a manual code change to point at a newer Overture release. Combined with OneMap's confirmed exhaustion (this file's other 2026-09-19 entry), there is nothing left for a recurring poll to find. Ran `schtasks /delete /tn SHIOK-GeocodeV2-Cycle /f`, confirmed deleted. `tmp/geocode_v2_cycle.py` and `tmp/geocode_v2_cycle_run.bat` remain on disk but inactive (no trigger); not deleted from disk, since decisions.md is append-only and code cleanup was not separately requested.

Owner approved promoting the single Overture-sourced candidate found earlier ("ye si approve"): postal 756935, lon 103.81298065 lat 1.46010005, source `OpenAddresses/Singapore Land Authority` via Overture. Completed the review pass the tool's own "candidate only until raw archive, hash/provenance, dedupe, coordinate validation, and attribution review pass" caveat requires, before promoting: ran `pipeline.overture_addresses` with `--archive-raw --confirm-overture-addresses`, producing a hashed raw archive at `raw/cded7259e2c1aedf9c2146d5ae4ae3fb107a6b37e3424257bca929eda20ab5ca/overture_addresses_sg_postcode_candidates.parquet` (123,883 rows, sha256 in the path itself); confirmed the coordinate falls well inside Singapore's bounding box (the report's own computed bbox: lat 1.207-1.470, lon 103.611-104.059); confirmed `address_rows=1` with a ~1.5m bounding-box spread (a precise single point, not an averaged/noisy cluster). Wrote `tmp/promote_756935.py` (one-off, mirrors `pipeline.geocode_universe.apply_success_to_row`'s coordinate-transform pattern but tags `coordinate_source=overture_candidate_sla_reviewed`, a distinct value from any OneMap-sourced record, preserving honest provenance) and ran it: wrote `processed/postal_universe_candidate_full_registered_geocoded_v4.parquet` (needs_geocode_after=474, ready_to_score_after=123969), input `..._v3.parquet` untouched. Scoring this postal is still deferred until the separately-running full rescore finishes, same reasoning as the 2026-09-18 promotion.

Separate, unscoped finding surfaced by the same `--archive-raw` run, flagged to the owner but explicitly NOT investigated or acted on: Overture's SG address data covers 123,883 of 124,443 (99.5%) of our whole postal universe, not just the 476-postal gap. Comparing Overture's independently-sourced coordinates against our EXISTING already-scored coordinates found 416 records with >100m deltas (93 with >250m, 23 with >1000m) -- meaning some already-`SCORED` records in the live universe may have coordinate accuracy issues Overture reveals. Separately, `comparison.new_from_overture=1433` -- 1,433 postcodes Overture has that are not in our current 124,443-postal universe at all, and `current_missing_from_overture=1993` -- 1,993 of our postal codes Overture does not have. This is a materially different, larger question (whole-universe coordinate QA and completeness) than the 476-postal NEEDS_GEOCODE gap this thread of work addressed, and would need its own separate scoping/approval before any action, following the same candidate-source-first + owner-approval pattern already established. This does not score, export, publish, mutate frozen v1, or touch the running rescore's inputs/outputs; it does not alter locked weights.

2026-09-19 - Coordinate-mismatch investigation Phase 1 executed (owner-approved, read-only):

Owner approved Phase 1 only of the 2026-09-19 coordinate-mismatch investigation scope (published as an HTML report via postplan, https://86rfn5ioiit0.postplan.dev). Categorized the existing 416-outlier list (from the same-day Overture comparison, no new API calls) by `coordinate_source`, then compared each source's outlier rate against its total population across the full 124,443-record universe (`processed/postal_universe_candidate_full_registered_geocoded_v3.parquet` column counts). Result, written to `qa/p2-1-gap-register/coordinate-mismatch-phase1-20260919.md`:

| coordinate_source | total | outliers>100m | outlier rate | delta max |
|---|---:|---:|---:|---:|
| osm_addr_postcode | 7,174 | 250 | 3.48% | 26,004.4m |
| postal_universe_onemap_2020 | 18,406 | 160 | 0.87% | 1,942.8m |
| hdb_existing_building | 13,436 | 5 | 0.04% | 222.2m |
| ura_no_dwelling_units | 83,432 | 1 | 0.001% | 109.5m |
| sla_dwelling_information | 1,420 | 0 | 0% | -- |
| onemap_search_bounded_geocode | 100 | 0 | 0% | -- |

Hypothesis ("one source disproportionately produces large deltas") confirmed strongly: `osm_addr_postcode` is 5.8% of the universe but 60.1% of all outliers, at 4x the outlier rate of the next-worst source and ~90x the rate of `hdb_existing_building`; it also uniquely produced the extreme tail (all 23 of the >1000m outliers, including both the 26km and 15.9km worst cases). `postal_universe_onemap_2020` has a smaller but non-trivial 0.87% outlier rate, worth a lower-priority Phase 2 pass. Phases 2-4 (manual verification, correction-policy decision, gated re-score) remain unapproved and unstarted; this entry covers Phase 1 only. This does not score, export, publish, mutate frozen v1, or touch the running rescore's inputs/outputs; it does not alter locked weights.

2026-09-19 - Coordinate-mismatch investigation Phase 2 executed, Phase 3 proposed (not decided), Phase 4 explicitly deferred:

Owner said "do phase 2 and 3 and 4 while waiting for rescore". Executed Phase 2 (manual verification of the 23 >1000m outliers) as scoped -- report at `qa/p2-1-gap-register/coordinate-mismatch-phase2-20260919.md`. Method: 3 records independently triangulated against third-party postcode-lookup sites via web search (streetdirectory.com/straitsdata.com for 079000, zip.nowmsg.com for 207663, ipostalcode.com for 757869 -- all three confirm Overture's coordinate and contradict our existing `osm_addr_postcode`-sourced one); the remaining 20 assessed via Singapore's public postal-district convention and specific landmark knowledge (no map/GIS tool used). Result: 11 of 23 have a confident-to-moderate verdict that Overture is correct and our existing coordinate is wrong (all 11 are `osm_addr_postcode`-sourced, consistent with Phase 1); 12 remain genuinely uncertain (deltas 1-3.8km where both coordinates plausibly sit in the same district) and are NOT given a forced verdict, including the one exception sourced from `postal_universe_onemap_2020` rather than `osm_addr_postcode` (postal 627871).

Phase 3 ("decide a correction policy") was explicitly scoped as an owner decision point, not something to execute unilaterally -- a proposal was presented to the owner (correct the 11 high/moderate-confidence records to Overture's coordinates tagged `coordinate_source=overture_verified_manual_review`; leave the 12 uncertain records and the remaining 393 sub-1000m outliers untouched pending further review) but has NOT yet been confirmed as of this entry.

Phase 4 ("corrected re-score") was explicitly NOT executed despite the owner's "do phase 2 and 3 and 4" instruction, because this investigation's own 2026-09-19 scope document states it "must not run alongside the current full rescore" -- and the full rescore (PID confirmed via Get-Process python) was still actively running at the time of this request. Flagged this constraint back to the owner rather than silently executing or silently skipping it. This does not score, export, publish, mutate frozen v1, or touch the running rescore's inputs/outputs; it does not alter locked weights.

2026-09-19 - Phase 3 corrections applied, Phase 4 completion-trigger registered (owner-approved, executed):

Owner approved the Phase 3 proposal ("green light, approve") and asked to schedule Phase 4 for right after the rescore finishes ("can we schedule phase 4 to after the rescoring? thats the purpose right" -- confirmed yes). Applied the 11 approved corrections via `tmp/apply_phase3_corrections.py`: wrote `processed/postal_universe_candidate_full_registered_geocoded_v5.parquet` (builds on v4, which already has postal 756935's promotion), each of the 11 records' `lat`/`lon`/`x`/`y` set to its Overture-verified value, `coordinate_source` set to `overture_verified_manual_review` (distinct from both the OneMap-sourced tag and the `overture_candidate_sla_reviewed` tag used for 756935, since these went through actual manual/triangulated review rather than being a bare candidate). Confirmed all 11 records had `status=READY_TO_SCORE` before correction (they already had data; only the coordinate value was wrong). The 12 genuinely-uncertain records and the other 393 sub-1000m outliers were explicitly left untouched, matching what was actually approved.

Registered a one-shot Windows Task Scheduler task `SHIOK-Phase4-Trigger` (`schtasks /create ... /sc MINUTE /mo 15`, checks every 15 minutes) running `tmp/phase4_trigger.py`. Each check reads `qa/revamp-r1/full-rescore-20260917/rescore.log` for a `"[STAGE 2 EXIT "` marker; if absent, exits quietly (tested manually, confirmed clean exit with the task still registered for the next check). Once the marker appears with exit code 0, it isolates the 12 owner-approved records (756935 + the 11 corrections) into their own throwaway single-purpose universe parquet under `qa/revamp-r1/phase4-trigger-20260919/`, runs `pipeline.score_batch --full-batch --confirm-full-batch` against that tiny 12-row universe using the now-final `processed/network_island.parquet`, writes a completion report, then deletes its own scheduled task (one-shot, unlike the deleted recurring geocode-retry loop -- this has a real, finite completion condition). If the rescore instead exits non-zero, the trigger logs the failure, does not attempt Phase 4, and still self-deletes rather than continuing to poll a failed run.

Explicit boundary, stated in the trigger script and here for the record: this automation will NEVER auto-export or auto-publish anything to the live site. It stops after writing the isolated score report. Exporting/publishing those 12 records into the actual live bundle remains a separate, explicitly-gated owner decision (`pipeline.export --confirm-export --confirm-live-score-export`), to be made after reviewing the trigger's report. This does not score, export, publish, mutate frozen v1, or touch the running rescore's inputs/outputs; it does not alter locked weights.

2026-09-19 - Owner walked back auto-publish, requested a guided manual post-rescore workflow instead:

Owner reconsidered ("oaky do delta, ignore what i say about auto") after being shown that publishing "just" the 12 Phase 3/4 records is not technically possible in isolation -- `pipeline.export` builds one complete, self-consistent site bundle from the whole scored universe, so any auto-publish would necessarily ship the entire freshly-rescored ~124k-record batch unreviewed, not just the small fix. Owner chose to keep the deliberate manual delta-review gate (already established earlier this session for Stage 3) rather than skip it.

Prepared `tmp/full_rescore_delta_report.py` (gitignored, not committed -- consistent with all other `tmp/` orchestration scripts this session) as the concrete tool for that review: a direct port of the proven `qa/p575_compare/p575_build_delta_report.py` subset-test pattern, retargeted from the 1200-record P575 pilot onto the full `processed/score_batches/full_rescore_20260917_232544/` output compared against the currently-published `web/public/data/generated_20260805_prefer_scored_routed/scores`. Computes state transitions, total-score delta distribution, bus zero-to-positive/positive-to-zero transitions (the P576 bus-remodel goal), covered-ratio drift (the network-conflation-repair goal), and four pass/fail assertions. Syntax-verified (`py_compile`, exit 0) but NOT run to completion: the fresh chunk files are 2.5-4GB combined, and attempting a full interactive run competed for disk I/O with the still-running rescore, so the attempt was killed rather than risk contention with the primary job. Will run for real once Stage 2 actually exits.

Documented the full manual post-rescore sequence for the owner: (1) confirm `[STAGE 2 EXIT 0]` in the rescore log; (2) run the delta report above; (3) manually review it -- particularly the four assertions and the largest-total-score-change sample; (4) if clean, run `pipeline.export` to a new versioned output directory; (5) merge in the Phase 4 trigger's separately-scored 12-record output (exact merge mechanics not yet verified -- flagged as unconfirmed rather than assumed); (6) `run.py readiness` and `run.py validate`; (7) explicit, manual, separately-approved publish decision -- no automation touches this step. This does not score, export, publish, mutate frozen v1, or touch the running rescore's inputs/outputs; it does not alter locked weights.

2026-09-21 - Full rescore Stage 2 crashed (exit -1) after 3.5 days, resumed:

`pipeline.score_batch` (the full-batch rescore's Stage 2) exited with code -1 at 21:30:14 SGT after ~3.5 days and ~213,000 CPU-seconds, having written 63 chunks (through record 348,245). Investigated root cause honestly: ruled out reboot (`Get-CimInstance Win32_OperatingSystem` shows last boot 2026-09-12, well before this run started), sleep/hibernate (no such `System` log events found near the crash time), disk exhaustion (167GB free on C:), and a Python-level exception (no traceback/error text anywhere in `rescore.log`, no Windows Error Reporting crash record, no relevant `System`/`Application` event-log entries in the 21:15-21:45 window). **Root cause remains unconfirmed** -- flagged honestly rather than guessed at.

The `SHIOK-Phase4-Trigger` safety design (2026-09-19 entry) worked exactly as intended on this real failure: detected the non-zero exit at 21:59:29 SGT, logged "NOT running Phase 4 automatically -- this needs manual review", and self-deleted (confirmed gone via `schtasks /query`, after one 30-second timeout on the first delete attempt that appears to have still succeeded). This is the first time this session one of the self-terminating trigger scripts actually encountered its failure path in production rather than only being tested for the success/not-yet-finished paths.

Confirmed `pipeline.score_batch --help` lists `--no-resume` as an explicit opt-out flag, meaning resume-by-default is the real, documented behavior; the original rescore command never passed that flag. Wrote `tmp/full-rescore-resume-20260921.ps1` (gitignored) that re-runs the *identical* `score_batch` invocation (same `--postal-universe`, `--network`, `--output-dir`; no `--no-resume`) without re-running Stage 1 (`network_island.parquet` is already valid and untouched). Launched via `Start-Process -WindowStyle Hidden` so it survives independently of the agent session, matching how the original run survived across days. Verified it is genuinely resuming and not restarting: chunk count stayed at 63 with unchanged timestamps while the new worker (confirmed via `Get-Process`) accumulated real CPU time. This does not score, export, publish, mutate frozen v1, or alter locked weights -- it restarts the already-approved one-attempt-only rescore exactly as it was running before the crash.

2026-09-22 - Rescore intentionally paused by owner ("i need to pause this shit, can?"):
